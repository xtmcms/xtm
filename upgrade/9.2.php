...
$video_config['play'] = "0";
$con_file = fopen(ENGINE_DIR . '/data/videoconfig.php', "w+") or die("Sorry, but cannot write the data in the file <b>engine/data/videoconfig.php.php</b>, it is not writable.<br />Please check the file permission (CHMOD)!");
...
clear_cache();
if ($db -> error_count) {
    $error_info = "Total scheduled queries: <b>" . $db -> query_num . "</b> has failed to execute query: <b>" . $db -> error_count . "</b>. Perhaps they have already been implemented previously.<br /><br /><div class=\"quote\"><b>Following queries cannot execute:</b><br /><br />";
    foreach ($db -> query_list as $value) {
        $error_info .= $value['query'] . "<br /><br />";
    } 
    $error_info .= "</div>";
} else $error_info = "";
msgbox("info", "Information", "<form action=\"index.php\" method=\"GET\">Upgrading database from version <b>9.2</b> to version <b>9.3</b> is  successfully completed.<br />{$error_info}<br />Click \"Next\" to continue<br /><br /><input type=\"hidden\" name=\"next\" value=\"next\"><input class=\"edit\" type=\"submit\" value=\"Next ...\"></form>");
?>