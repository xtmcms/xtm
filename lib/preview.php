...
if ($config['allow_site_wysiwyg']) $allow_br = 0;
else $allow_br = 1;
...
if (preg_match("#\\{title limit=['\"](.+?)['\"]\\}#i", $tpl->copy_template, $matches)) {
    ...
    if ($count AND xtm_strlen($title, $config['charset']) > $count) {
        $title = xtm_substr($title, 0, $count, $config['charset']);
        if (($temp_dmax = xtm_strrpos($title, ' ', $config['charset']))) $title = xtm_substr($title, 0, $temp_dmax, $config['charset']);
    }
    $tpl->set($matches[0], $title);
}
$c_list = implode(',', $_REQUEST['catlist']);
if (strpos($tpl->copy_template, "[catlist=") !== false) {
    $tpl->copy_template = preg_replace("#\\[catlist=(.+?)\\](.*?)\\[/catlist\\]#ies", "check_category('\\1', '\\2', '{$c_list}')", $tpl->copy_template);
}
if (strpos($tpl->copy_template, "[not-catlist=") !== false) {
    $tpl->copy_template = preg_replace("#\\[not-catlist=(.+?)\\](.*?)\\[/not-catlist\\]#ies", "check_category('\\1', '\\2', '{$c_list}', false)", $tpl->copy_template);
}
$tpl->set('{views}', 0);
