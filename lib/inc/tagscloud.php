...
if ($_POST['action'] == "mass_delete") {
    ...
    foreach ($selected_tags as $name) {
        ...
        if (!$name) {
            msg("error", $lang['mass_error'], $lang['mass_tags_err_1'], "?mod=tagscloud&start_from={$start_from}");
            die();
        }
        ...
        if (count($tag_array)) {
            ...
            while ($row = $db->get_row($sql_result)) {
                ...
                if (count($tags)) $post_tags = @$db->safesql(implode(", ", $tags));
                else $post_tags = "";
...
$(function(){
		...
		$('.dellink').click(function(){
		    ...
		    xtmconfirm( '{$lang['tagscloud_del']} <b>&laquo;'+tag_name+'&raquo;</b> {$lang['tagscloud_del_1']}', '{$lang['p_confirm']}', function () {

				document.location='?mod=tagscloud&start_from={$start_from}&user_hash={$xtm_login_hash}&action=delete&name=' + encodeURIComponent(tag_name) + '';
		...
		$('.editlink').click(function(){
			...
			xtmprompt('{$lang['tagscloud_edit_1']}', tag_name, '{$lang['tagscloud_edit']}', function (r) {
				if (tag_name != r) {	
					document.location='?mod=tagscloud&start_from={$start_from}&user_hash={$xtm_login_hash}&action=edit&oldname=' + encodeURIComponent(tag_name) + '&newname=' + encodeURIComponent(r);
				}		
			});
