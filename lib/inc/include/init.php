...
$_IP = get_ip();
$_TIME = time() + ($config['date_adust'] * 60);
...
if ($allow_login) {
    if ($config['extra_login']) {
        ...
        $post = true;
        $check_log = true;
    } elseif (intval($_SESSION['xtm_user_id']) > 0 AND $_SESSION['xtm_password']) {
        $username = $_SESSION['xtm_user_id'];
        $cmd5_password = $_SESSION['xtm_password'];
        $post = false;
        if (!$_SESSION['check_log']) $check_log = true;
    } elseif (intval($_COOKIE['xtm_user_id']) > 0 AND $_COOKIE['xtm_password']) {
        $username = $_COOKIE['xtm_user_id'];
        $cmd5_password = $_COOKIE['xtm_password'];
        $post = false;
        $check_log = true;
    }
...
if (check_login($username, $cmd5_password, $post, $check_log)) {
    $is_loged_in = true;
    $xtm_login_hash = md5($_SERVER['HTTP_HOST'] . $member_id['user_id'] . sha1($cmd5_password) . $config['key'] . date("Ymd"));
    if (!$_SESSION['xtm_user_id'] and $_COOKIE['xtm_user_id']) {
        $_SESSION['xtm_user_id'] = $_COOKIE['xtm_user_id'];
...
if ($is_loged_in and $config['log_hash'] and (($_COOKIE['xtm_hash'] != $member_id['hash']) or ($member_id['hash'] == ""))) {
    $is_loged_in = false;
}
