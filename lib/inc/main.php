...
set_cookie("xtm_compl", $row['count'], 365);
if ($c_complaint AND $user_group[$member_id['user_group']]['admin_complaint']) {
    ...
    if ($row['count'] > intval($_COOKIE['xtm_compl'])) {
        $c_complaint .= <<<HTML
<script language="javascript" type="text/javascript">
<!--

$(function(){
	xtmconfirm( '{$lang['opt_complaint_20']}', '{$lang['p_confirm']}', function () {
...
function check_files ( folder ){
	...
	if (folder == "snap") {
	    ...
	    xtmconfirm( '{$lang['anti_snapalert']}', '{$lang['p_confirm']}', function () {
...
HTML;
    echo "<br /><input id=\"check_updates\" name=\"check_updates\" class=\"btn btn-inverse\" type=\"button\" value=\"{$lang['xtm_udate']}\">&nbsp;<input id=\"clearbutton\" name=\"clearbutton\" class=\"btn btn-danger\" type=\"button\" value=\"{$lang['btn_clearcache']}\">";
...
<table width="100%">
    <tr>
        <td style="padding:2px;">{$lang['xtm_version']}</td>
...
function xtm_activation ( code ){

	document.getElementById( 'result_info' ).innerHTML = '{$lang['nl_sinfo']}';

	if (code == 'key') {

		var xtm_key = document.getElementById('sitekey').value ;
		var varsString = "xtm_key=" + xtm_key;
...
HTML;
        echo "<br /><table width=\"100%\"align=center><tr><td style='padding:3px; border:1px dashed rgb(190,190,190); background-color:lightyellow;'>
         <div id=\"xtm-activation\">$lang[trial_info]<br /><br /><b>$lang[trial_key]</b><span style=\"padding-left:7px;\"><input class=\"edit bk\" type=\"text\" size=\"45\" name=\"sitekey\" id=\"sitekey\"> <input class=\"btn btn-success btn-mini\" type=\"button\" onClick=\"xtm_activation( 'key' ); return false;\" value=\"$lang[trial_act]\"></span><div id=\"result_info\" style=\"color:red;\"><br /><span class=\"navigation\">{$lang['key_format']} <b>XXXXX-XXXXX-XXXXX-XXXXX-XXXXX</b></span></div></div>
