		...
		$('#xtmuserpopup').dialog({
			...
			buttons: {
				...
				"{$lang['edit_dnews']}": function() { 
					window.frames.edituserframe.confirmDelete("$PHP_SELF?mod=editusers&action=dodeleteuser&popup=yes&id=" + id + "&user_hash={$xtm_login_hash}");
				},
			...
			open: function(event, ui) { 
				$("#xtmuserpopup").html("<iframe name='edituserframe' id='edituserframe' width='100%' height='400' src='{$PHP_SELF}?mod=editusers&action=edituser&id=" + id + "&rndval=" + rndval + "' frameborder='0' marginwidth='0' marginheight='0' allowtransparency='true'></iframe>");
			},
...
HTML;
	...
	function confirmdelete(id, user){
	    xtmconfirm( "' . $lang['user_deluser'] . '", "' . $lang['p_confirm'] . '", function () {
		    document.location="' . $PHP_SELF . '?mod=editusers&user_hash=' . $xtm_login_hash . '&action=dodeleteuser&id="+id+"&user="+user;
		} );
    }
...
HTML;
    if ($_REQUEST['search_reglevel']) {
        $search_reglevel = $_REQUEST['search_reglevel'];
        $group_list = get_groups($_REQUEST['search_reglevel']);
    } else $group_list = get_groups();
    if ($_REQUEST['search_banned'] == "yes") {
        $search_banned = "yes";
        $ifch = "checked";
    }
...
<!--
function cdelete(id){
	    xtmconfirm( '{$lang['comm_alldelconfirm']}', '{$lang['p_confirm']}', function () {
			document.location='?mod=editusers&action=dodelcomments&user_hash={$xtm_login_hash}&id=' + id + '';
		} );
}
...
menu[1]='<a onClick="javascript:cdelete(' + m_id + '); return(false)" href="?mod=editusers&action=dodelcomments&user_hash={$xtm_login_hash}&id=' + m_id + '" >{$lang['comm_del']}</a>';
...
HTML;
    ...
    while ($row = $db->get_row()) {
        ...
        $user_delete = "[<a class=maintitle onClick=\"javascript:confirmdelete('" . $row[user_id] . "', '" . $row[name] . "'); return(false)\"  href=\"" . $PHP_SELF . "?mod=editusers&user_hash={$xtm_login_hash}&action=dodeleteuser&id=" . $row[user_id] . "&user=" . $row[name] . "\">" . $lang[user_del] . "</a>]";
...
HTML;
    ...
    if (!$row['user_id']) die("User not found");
    if ($member_id['user_group'] != 1 AND $row['user_group'] == 1) die($lang['edit_not_admin']);
...
function confirmDelete(url){
	...
	parent.xtmconfirm( '{$lang['user_deluser']}', '{$lang['p_confirm']}', function () {
...
HTML;
    ...
    if ($member_id['user_group'] != 1 AND $editlevel < 2) die($lang['admin_not_access']);
    ...
    if ($_POST['editicq']) $editicq = intval($_POST['editicq']);
    else $editicq = "";
    ...
    if ($editmail != $row['email']) {
        if ($db->num_rows($db->query("SELECT user_id FROM " . USERPREFIX . "_users WHERE email = '$editmail'"))) header("Location: {$_SERVER['REQUEST_URI']}");
    }
    ...
    if (!$row['user_id']) die("User not found");
    if ($member_id['user_group'] != 1 AND $row['user_group'] == 1) die($lang['user_undel']);
