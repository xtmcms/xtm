...
if ($_GET['action'] == "add") {
    if ($_POST['user_hash'] == "" or $_POST['user_hash'] != $xtm_login_hash) {
    ...
    foreach ($category as $value) {
        if ($value == "all") $category_list[] = $value;
        else $category_list[] = intval($value);
    }
    ...
    if ($canedit == false) {
        echo "<form method=post action=\"?mod=addvote&action=add\" style=\"padding:0; margin:0\" name=\"addvote\" onsubmit=\"if(document.addvote.title.value == '' || document.addvote.body.value == ''){xtmalert('$lang[vote_alert]', '$lang[p_info]');return false}\">";
        $button = "<input type=\"submit\" class=\"btn btn-success\" value=\"&nbsp;&nbsp;{$lang['vote_new']}&nbsp;&nbsp;\">";
    } else {
        echo "<form method=post action=\"?mod=addvote&action=update&id=$id\" style=\"padding:0; margin:0\" name=\"addvote\" onsubmit=\"if(document.addvote.title.value == '' || document.addvote.body.value == ''){xtmalert('$lang[vote_alert]', '$lang[p_info]');return false}\">";
