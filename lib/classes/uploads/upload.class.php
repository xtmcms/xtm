    ...
    function getFileSize()
    {
        ...
        if ($url) {
            ...
            if ($fp) {
                ...
                fclose($fp);
                if (preg_match("#Content-Length: ([0-9]+)#i", $x, $size)) return intval($size[1]);
                else return 0;
            } else return 0;
        } else return 0;
    }
        ...
        if ($error_code !== UPLOAD_ERR_OK) {
            switch ($error_code) {
                case UPLOAD_ERR_INI_SIZE:
                    $error_code = 'PHP Error: The uploaded file exceeds the upload_max_filesize directive in php.ini';
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    $error_code = 'PHP Error: The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form';
                    break;
                case UPLOAD_ERR_PARTIAL:
                    $error_code = 'PHP Error: The uploaded file was only partially uploaded';
                    break;
                case UPLOAD_ERR_NO_FILE:
                    $error_code = 'PHP Error: No file was uploaded';
                    break;
                case UPLOAD_ERR_NO_TMP_DIR:
                    $error_code = 'PHP Error: Missing a PHP temporary folder';
                    break;
                case UPLOAD_ERR_CANT_WRITE:
                    $error_code = 'PHP Error: Failed to write file to disk';
                    break;
                case UPLOAD_ERR_EXTENSION:
                    $error_code = 'PHP Error: File upload stopped by extension';
                    break;
                default:
...
class FileUploader
{
    ...
    function __construct($area, $news_id, $author, $t_size, $t_seite, $make_thumb = true, $make_watermark = true, $flash_mode = false)
    {
        ...
        if (@ini_get('safe_mode') == 1) define('FOLDER_PREFIX', "");
        else define('FOLDER_PREFIX', date("Y-m") . "/");
    }
    ...
    function FileUpload()
    {
        ...
        if ($config['files_allow'] == "yes" AND $user_group[$member_id['user_group']]['allow_file_upload'] AND in_array($type, $this->allowed_files)) {
            ...
            if ($this->area != "template" AND $user_group[$member_id['user_group']]['max_images']) {
                ...
                if ($row['images']) $count_images = count(explode("|||", $row['images']));
                else $count_images = false;
            ...
            if ($uploaded_filename) {
                ...
                if ($this->make_watermark OR $config['max_up_side']) {
                    ...
                    if ($config['max_up_side']) $i_info = @getimagesize(ROOT_DIR . "/uploads/posts/" . FOLDER_PREFIX . $uploaded_filename);
                }
