...
class ParseFilter
{
    ...
    var $allowed_domains = array("http://youtube.com/", "http://www.youtube.com/", "http://maps.google.ru/", "http://maps.google.com/", "http://player.vimeo.com/", "http://www.facebook.com/", "http://www.dailymotion.com/", "http://www.bing.com/", "http://www.ustream.tv/", "http://w.soundcloud.com/", "http://www.coveritlive.com/", "https://maps.google.com/");
    ...
    function filterAttr($attrSet, $tagName)
    {
        ...
        for ($i = 0; $i < count($attrSet); $i++) {
            ...
            if ((!$attrFound && $this->attrMethod) || ($attrFound && !$this->attrMethod)) {
                ...
                elseif ($attrSubSet[1] == "0") $newSet[] = $attrSubSet[0] . '="0"'; else $newSet[] = $attrSubSet[0] . '=""';
            }
        }
        ;
        return $newSet;
    ...
    function BB_Parse($source, $use_html = true)
    {
        ...
        foreach ($smilies_arr as $smile) {
            $smile = trim($smile);
            $find[] = "':$smile:'";
            $replace[] = "<!--smile:{$smile}--><img style=\"vertical-align: middle;border: none;\" alt=\"$smile\" src=\"" . $config['http_home_url'] . "lib/data/emoticons/{$smile}.gif\" /><!--/smile-->";
        }
        ...
        if ($count_start AND $count_start == $count_end) {
            ...
            if (!$this->allow_code) {
                $source = preg_replace("#\[quote\](.+?)\[/quote\]#ies", "\$this->clear_div_tag(array('title' => '', 'txt' => '\\1'), 'quote')", $source);
                $source = preg_replace("#\[quote=(.+?)\](.+?)\[/quote\]#ies", "\$this->clear_div_tag(array('title' => '\\1', 'txt' => '\\2'), 'quote')", $source);
            }
        ...
        if ($count_start AND $count_start == $count_end) {
            ...
            $source = preg_replace("#\[/spoiler\]#i", "<!--spoiler_text_end--></div><!--/xtm_spoiler-->", $source);
        }
        ...
        if (!$this->safe_mode) {
            ...
            $source = preg_replace("#\[ol=([^\]]+)\](.+?)\[\*\]#ies", "\$this->build_list('\\1')", $source);
            $source = str_ireplace("[list][*]", "<!--xtm_list--><ul><li>", $source);
            $source = preg_replace("#\[list\](.+?)\[\*\]#is", "<!--xtm_list--><ul><li>", $source);
            $source = str_replace("[*]", "</li><!--xtm_li--><li>", $source);
            $source = str_ireplace("[/list]", "</li></ul><!--xtm_list_end-->", $source);
            $source = str_ireplace("[/ol]", "</li></ol><!--xtm_list_end-->", $source);
    ...
    function decodeBBCodes($txt, $use_html = true, $wysiwig = false)
    {
        ...
        $txt = preg_replace("#<!--xtm_leech_begin--><a href=[\"'](http://|https://|ftp://|ed2k://|news://|magnet:)?(\S.+?)['\"].*?" . ">(.+?)</a><!--xtm_leech_end-->#ie", "\$this->decode_leech('\\1\\2', '\\3')", $txt);
        $txt = preg_replace("#<!--xtm_video_begin-->(.+?)src=\"(.+?)\"(.+?)<!--xtm_video_end-->#is", '[video=\\2]', $txt);
        $txt = preg_replace("#<!--xtm_video_begin:(.+?)-->(.+?)<!--xtm_video_end-->#is", '[video=\\1]', $txt);
        $txt = preg_replace("#<!--xtm_audio_begin:(.+?)-->(.+?)<!--xtm_audio_end-->#is", '[audio=\\1]', $txt);
        $txt = preg_replace("#<!--xtm_image_begin:(.+?)-->(.+?)<!--xtm_image_end-->#ies", "\$this->decode_xtm_img('\\1')", $txt);
        $txt = preg_replace("#<!--xtm_youtube_begin:(.+?)-->(.+?)<!--xtm_youtube_end-->#is", '[media=\\1]', $txt);
        $txt = preg_replace("#<!--xtm_media_begin:(.+?)-->(.+?)<!--xtm_media_end-->#is", '[media=\\1]', $txt);
        $txt = preg_replace("#<!--xtm_flash_begin:(.+?)-->(.+?)<!--xtm_flash_end-->#ies", "\$this->decode_flash('\\1')", $txt);
        $txt = preg_replace("#<!--xtm_spoiler-->(.+?)<!--spoiler_text-->#is", '[spoiler]', $txt);
        $txt = preg_replace("#<!--xtm_spoiler (.+?) -->(.+?)<!--spoiler_text-->#is", '[spoiler=\\1]', $txt);
        $txt = str_replace("<!--spoiler_text_end--></div><!--/xtm_spoiler-->", '[/spoiler]', $txt);
        $txt = str_replace("<!--xtm_list--><ul><li>", "[list]\n[*]", $txt);
        $txt = str_replace("</li></ul><!--xtm_list_end-->", '[/list]', $txt);
        $txt = str_replace("</li></ol><!--xtm_list_end-->", '[/ol]', $txt);
        $txt = str_replace("</li><!--xtm_li--><li>", '[*]', $txt);
        $txt = str_replace("<pre><code>", '[code]', $txt);
        $txt = str_replace("</code></pre>", '[/code]', $txt);
        $txt = preg_replace("#<!--xtm_ol_(.+?)-->(.+?)<!--/xtm_ol-->#i", "[ol=\\1]\n[*]", $txt);
        if (!$wysiwig) {
    ...
    function build_list($type)
    {
        ...
        if (in_array($type, $allowed_types)) return "<!--xtm_ol_{$type}--><ol type=\"{$type}\"><li><!--/xtm_ol-->";
        else return "<!--xtm_ol_1--><ol type=\"1\"><li><!--/xtm_ol-->";
    }
    ...
    function build_flash($size, $url)
    {
        ...
        return "<!--xtm_flash_begin:{$width}||{$height}||{$url}--><object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' width='$width' height='$height'><param name='movie' value='$url'><param name='wmode' value='transparent' /><param name='play' value='true'><param name='loop' value='true'><param name='quality' value='high'><param name='allowscriptaccess' value='never'><embed AllowScriptAccess='never' src='$url' width='$width' height='$height' play='true' loop='true' quality='high' wmode='transparent'></embed></object><!--xtm_flash_end-->";
    }
    ...
    function build_media($url)
    {
        ...
        if ($source['host'] == "youtube.com" OR $source['host'] == "youtu.be") {
            if ($this->video_config['tube_xtm']) {
                ...
                if ($this->video_config['flv_watermark']) $watermark = "&amp;showWatermark=true&amp;watermarkPosition={$this->video_config['flv_watermark_pos']}&amp;watermarkMargin=0&amp;watermarkAlpha={$this->video_config['flv_watermark_al']}&amp;watermarkImageUrl={THEME}/xtmimages/flv_watermark.png";
                else $watermark = "&showWatermark=false";
                $preview = "&amp;showPreviewImage=true&amp;previewImageUrl=";
                if ($this->video_config['preview']) $preview = "&amp;showPreviewImage=true&amp;previewImageUrl={THEME}/xtmimages/youtubepreview.jpg";
                ...
                return "<!--xtm_media_begin:{$decode_url}--><object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0\" width=\"{$width}\" height=\"{$height}\" id=\"Player-{$id_player}\">
					<param name=\"movie\" value=\"" . $config['http_home_url'] . "lib/classes/flashplayer/media_player_v3.swf?stageW={$width}&amp;stageH={$height}&amp;contentType=video&amp;videoUrl=http://www.youtube.com/watch?v={$video_link}{$watermark}{$preview}&amp;youTubePlaybackQuality={$this->video_config['youtube_q']}&amp;isYouTube=true&amp;rollOverAlpha=0.5&amp;contentBgAlpha=0.8&amp;progressBarColor={$this->video_config['progressBarColor']}&amp;defaultVolume=1&amp;fullSizeView=3&amp;showRewind=false&amp;showInfo=false&amp;showFullscreen=true&amp;showScale=true&amp;showSound=true&amp;showTime=true&amp;showCenterPlay=true{$autohide}&amp;videoLoop=false&amp;defaultBuffer={$this->video_config['buffer']}\" />
					<param name=\"allowFullScreen\" value=\"true\" />
					<param name=\"scale\" value=\"noscale\" />
					<param name=\"quality\" value=\"high\" />
					<param name=\"bgcolor\" value=\"#000000\" />
					<param name=\"wmode\" value=\"opaque\" />
					<embed src=\"" . $config['http_home_url'] . "lib/classes/flashplayer/media_player_v3.swf?stageW={$width}&amp;stageH={$height}&amp;contentType=video&amp;videoUrl=http://www.youtube.com/watch?v={$video_link}{$watermark}{$preview}&amp;youTubePlaybackQuality={$this->video_config['youtube_q']}&amp;isYouTube=true&amp;rollOverAlpha=0.5&amp;contentBgAlpha=0.8&amp;progressBarColor={$this->video_config['progressBarColor']}&amp;defaultVolume=1&amp;fullSizeView=3&amp;showRewind=false&amp;showInfo=false&amp;showFullscreen=true&amp;showScale=true&amp;showSound=true&amp;showTime=true&amp;showCenterPlay=true{$autohide}&amp;videoLoop=false&amp;defaultBuffer={$this->video_config['buffer']}\" quality=\"high\" bgcolor=\"#000000\" wmode=\"opaque\" allowFullScreen=\"true\" width=\"{$width}\" height=\"{$height}\" align=\"middle\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\"></embed>
					</object><!--xtm_media_end-->";
            } else return '<!--xtm_media_begin:' . $decode_url . '--><iframe title="YouTube video player" width="' . $width . '" height="' . $height . '" src="http://www.youtube.com/embed/' . $video_link . '?rel=' . intval($this->video_config['tube_related']) . '&amp;wmode=transparent" frameborder="0" allowfullscreen></iframe><!--xtm_media_end-->';
            ...
            return '<!--xtm_media_begin:' . $decode_url . '--><object id="smotriComVideoPlayer" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="' . $width . '" height="' . $height . '"><param name="movie" value="http://pics.smotri.com/player.swf?file=' . $video_link . '&amp;bufferTime=3&amp;autoStart=false&amp;str_lang=rus&amp;xmlsource=http%3A%2F%2Fpics.smotri.com%2Fcskins%2Fblue%2Fskin_color.xml&xmldatasource=http%3A%2F%2Fpics.smotri.com%2Fskin_ng.xml" /><param name="allowScriptAccess" value="always" /><param name="allowFullScreen" value="true" /><param name="wmode" value="opaque" /><embed src="http://pics.smotri.com/player.swf?file=' . $video_link . '&amp;bufferTime=3&amp;autoStart=false&amp;str_lang=rus&amp;xmlsource=http%3A%2F%2Fpics.smotri.com%2Fcskins%2Fblue%2Fskin_color.xml&amp;xmldatasource=http%3A%2F%2Fpics.smotri.com%2Fskin_ng.xml" quality="high" allowscriptaccess="always" allowfullscreen="true" wmode="opaque"  width="' . $width . '" height="' . $height . '" type="application/x-shockwave-flash"></embed></object><!--xtm_media_end-->';
            ...
            return '<!--xtm_media_begin:' . $decode_url . '--><iframe width="' . $width . '" height="' . $height . '" src="http://player.vimeo.com/video/' . $video_link . '" frameborder="0" allowfullscreen></iframe><!--xtm_media_end-->';
            ...
            return '<!--xtm_media_begin:' . $decode_url . '--><embed src="http://media.mtvnservices.com/mgid:moses:video:gametrailers.com:' . $video_link . '" width="' . $width . '" height="' . $height . '" type="application/x-shockwave-flash" allowFullScreen="true" allowScriptAccess="always" base="." flashVars="" wmode="opaque"></embed><!--xtm_media_end-->';
    ...
    function build_url($url = array())
    {
        ...
        if ($this->safe_mode) {
            ...
            if (strlen(trim($url['show'])) < 3) return "[url=" . $url['html'] . "]" . $url['show'] . "[/url]";
        }
        ...
        if ($url['leech']) {
            $url['html'] = $config['http_home_url'] . "lib/go.php?url=" . rawurlencode(base64_encode($url['html']));
            return "<!--xtm_leech_begin--><a href=\"" . $url['html'] . "\" " . $target . ">" . $url['show'] . "</a><!--xtm_leech_end-->" . $url['end'];
        } else {
            if ($this->safe_mode AND !$config['allow_search_link']) return "<a href=\"" . $url['html'] . "\" " . $target . " rel=\"nofollow\">" . $url['show'] . "</a>" . $url['end'];
            else return "<a href=\"" . $url['html'] . "\" " . $target . ">" . $url['show'] . "</a>" . $url['end'];
        }
    }
    ...
    function build_video($url)
    {
        ...
        if (stripos($url, "rtmp:") === false) $detect_rtmp = false;
        else $detect_rtmp = true;
        if ($type == "flv" or $type == "mp4" or $type == "m4v" or $type == "m4a" or $type == "mov" or $type == "3gp" or $type == "f4v" or $detect_rtmp) {
            if ($this->video_config['flv_watermark']) $watermark = "&amp;showWatermark=true&amp;watermarkPosition={$this->video_config['flv_watermark_pos']}&amp;watermarkMargin=0&amp;watermarkAlpha={$this->video_config['flv_watermark_al']}&amp;watermarkImageUrl={THEME}/xtmimages/flv_watermark.png";
            else $watermark = "&amp;showWatermark=false";
            $preview = "&amp;showPreviewImage=true&amp;previewImageUrl=";
            if ($this->video_config['preview']) $preview = "&amp;showPreviewImage=true&amp;previewImageUrl={THEME}/xtmimages/videopreview.jpg";
            if ($this->video_config['startframe']) $preview = "&amp;showPreviewImage=false";
            if ($option[1] != "") {
            ...
            return "<!--xtm_video_begin:{$decode_url}--><object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0\" width=\"{$width}\" height=\"{$height}\" id=\"Player-{$id_player}\">
				<param name=\"movie\" value=\"" . $config['http_home_url'] . "lib/classes/flashplayer/media_player_v3.swf?stageW={$width}&amp;stageH={$height}&amp;contentType=video{$video_url}{$watermark}{$preview}&amp;isYouTube=false&amp;rollOverAlpha=0.5&amp;contentBgAlpha=0.8&amp;progressBarColor={$this->video_config['progressBarColor']}&amp;defaultVolume=1&amp;fullSizeView={$this->video_config['fullsizeview']}&amp;showRewind=false&amp;showInfo=false&amp;showFullscreen=true&amp;showScale=true&amp;showSound=true&amp;showTime=true&amp;showCenterPlay=true{$autohide}&amp;videoLoop=false&amp;defaultBuffer={$this->video_config['buffer']}\" />
				<param name=\"allowFullScreen\" value=\"true\" />
				<param name=\"scale\" value=\"noscale\" />
				<param name=\"quality\" value=\"high\" />
				<param name=\"bgcolor\" value=\"#000000\" />
				<param name=\"wmode\" value=\"opaque\" />
				<embed src=\"" . $config['http_home_url'] . "lib/classes/flashplayer/media_player_v3.swf?stageW={$width}&amp;stageH={$height}&amp;contentType=video{$video_url}{$watermark}{$preview}&amp;isYouTube=false&amp;rollOverAlpha=0.5&amp;contentBgAlpha=0.8&amp;progressBarColor={$this->video_config['progressBarColor']}&amp;defaultVolume=1&amp;fullSizeView={$this->video_config['fullsizeview']}&amp;showRewind=false&amp;showInfo=false&amp;showFullscreen=true&amp;showScale=true&amp;showSound=true&amp;showTime=true&amp;showCenterPlay=true{$autohide}&amp;videoLoop=false&amp;defaultBuffer={$this->video_config['buffer']}\" quality=\"high\" bgcolor=\"#000000\" wmode=\"opaque\" allowFullScreen=\"true\" width=\"{$width}\" height=\"{$height}\" align=\"middle\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\"></embed>
				</object><!--xtm_video_end-->";
        } elseif ($type == "avi" OR $type == "divx" OR $type == "mkv") {
            $url = htmlspecialchars(trim($url), ENT_QUOTES, $config['charset']);
            return "<!--xtm_video_begin:{$decode_url}--><object classid=\"clsid:67DABFBF-D0AB-41fa-9C46-CC0F21721616\" width=\"{$width}\" height=\"{$height}\" codebase=\"http://go.divx.com/plugin/DivXBrowserPlugin.cab\">
            ...
            $url = htmlspecialchars(trim($url), ENT_QUOTES, $config['charset']);
            return "<!--xtm_video_begin:{$decode_url}--><object id=\"mediaPlayer\" width=\"{$width}\" height=\"{$height}\" classid=\"CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6\" standby=\"Loading Microsoft Windows Media Player components...\" type=\"application/x-oleobject\">
    ...
    function build_audio($url)
    {
        ...
        $id_player = md5(microtime());
        $this->video_config['buffer'] = intval($this->video_config['buffer']);
        return "<!--xtm_audio_begin:{$decode_url}--><object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0\" width=\"{$width}\" height=\"{$height}\" id=\"Player-{$id_player}\">
				<param name=\"movie\" value=\"" . $config['http_home_url'] . "lib/classes/flashplayer/media_player_v3.swf?stageW={$width}&amp;stageH={$height}&amp;contentType=audio&amp;videoUrl={$url}&amp;showWatermark=false&amp;showPreviewImage=true&amp;previewImageUrl=&amp;autoPlays={$this->video_config['play']}&amp;isYouTube=false&amp;rollOverAlpha=0.5&amp;contentBgAlpha=0.8&amp;progressBarColor={$this->video_config['progressBarColor']}&amp;defaultVolume=1&amp;fullSizeView=2&amp;showRewind=false&amp;showInfo=false&amp;showFullscreen=true&amp;showScale=false&amp;showSound=true&amp;showTime=true&amp;showCenterPlay=false&amp;autoHideNav=false&amp;videoLoop=false&amp;defaultBuffer={$this->video_config['buffer']}\" />
				<param name=\"allowFullScreen\" value=\"false\" />
				<param name=\"scale\" value=\"noscale\" />
				<param name=\"quality\" value=\"high\" />
				<param name=\"bgcolor\" value=\"#000000\" />
				<param name=\"wmode\" value=\"opaque\" />
				<embed src=\"" . $config['http_home_url'] . "lib/classes/flashplayer/media_player_v3.swf?stageW={$width}&amp;stageH={$height}&amp;contentType=audio&amp;videoUrl={$url}&amp;showWatermark=false&amp;showPreviewImage=true&amp;previewImageUrl=&amp;autoPlays={$this->video_config['play']}&amp;isYouTube=false&amp;rollOverAlpha=0.5&amp;contentBgAlpha=0.8&amp;progressBarColor={$this->video_config['progressBarColor']}&amp;defaultVolume=1&amp;fullSizeView=2&amp;showRewind=false&amp;showInfo=false&amp;showFullscreen=true&amp;showScale=false&amp;showSound=true&amp;showTime=true&amp;showCenterPlay=false&amp;autoHideNav=false&amp;videoLoop=false&amp;defaultBuffer={$this->video_config['buffer']}\" quality=\"high\" bgcolor=\"#000000\" wmode=\"opaque\" allowFullScreen=\"false\" width=\"{$width}\" height=\"{$height}\" align=\"middle\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\"></embed>
				</object><!--xtm_audio_end-->";
    }
        ...
        if (intval($config['tag_img_width'])) {
            if (clean_url($config['http_home_url']) != clean_url($url)) {
                ...
                if ($img_info[0] > $config['tag_img_width']) {
                    ...
                    if ($align == '') return "<!--xtm_image_begin:{$info}--><a href=\"{$url}\" onclick=\"return hs.expand(this)\" ><img src=\"$url\" width=\"{$config['tag_img_width']}\" height=\"{$out_heigh}\" {$alt} /></a>{$caption}<!--xtm_image_end-->";
                    else return "<!--xtm_image_begin:{$info}--><a href=\"{$url}\" onclick=\"return hs.expand(this)\" ><img src=\"$url\" width=\"{$config['tag_img_width']}\" height=\"{$out_heigh}\" style=\"float:{$align};\" {$alt} /></a>{$caption}<!--xtm_image_end-->";
                }
            }
        }
        if ($align == '') return "<!--xtm_image_begin:{$info}--><img src=\"{$url}\" {$alt} /><!--xtm_image_end-->";
        else return "<!--xtm_image_begin:{$info}--><img src=\"{$url}\" style=\"float:{$align};\" {$alt} /><!--xtm_image_end-->";
    }

    function decode_xtm_img($txt)
    {
        $txt = stripslashes($txt);
        $txt = explode("|", $txt);
        $url = $txt[0];
    ...
    function clear_div_tag($spoiler = array(), $tag = "spoiler")
    {
        $spoiler['txt'] = str_replace('\"', '"', $spoiler['txt']);
        $spoiler['title'] = str_replace('\"', '"', $spoiler['title']);
        $spoiler['txt'] = preg_replace('/<div[^>]*>/', '', $spoiler['txt']);
        $spoiler['txt'] = str_replace("</div>", "<br />", $spoiler['txt']);
        if ($spoiler['title']) return "[{$tag}={$spoiler['title']}]" . $spoiler['txt'] . "[/{$tag}]";
        else return "[{$tag}]" . $spoiler['txt'] . "[/{$tag}]";
    }
    ...
    function build_spoiler($title = "")
    {
        ...
        if (!$title) {
            return "<!--xtm_spoiler--><div class=\"title_spoiler\"><img id=\"image-" . $id_spoiler . "\" style=\"vertical-align: middle;border: none;\" alt=\"\" src=\"{THEME}/xtmimages/spoiler-plus.gif\" />&nbsp;<a href=\"javascript:ShowOrHide('" . $id_spoiler . "')\"><!--spoiler_title-->" . $lang['spoiler_title'] . "<!--spoiler_title_end--></a></div><div id=\"" . $id_spoiler . "\" class=\"text_spoiler\" style=\"display:none;\"><!--spoiler_text-->";
        } else {
            return "<!--xtm_spoiler $title --><div class=\"title_spoiler\"><img id=\"image-" . $id_spoiler . "\" style=\"vertical-align: middle;border: none;\" alt=\"\" src=\"{THEME}/xtmimages/spoiler-plus.gif\" />&nbsp;<a href=\"javascript:ShowOrHide('" . $id_spoiler . "')\"><!--spoiler_title-->" . $title . "<!--spoiler_title_end--></a></div><div id=\"" . $id_spoiler . "\" class=\"text_spoiler\" style=\"display:none;\"><!--spoiler_text-->";
        }
    }
