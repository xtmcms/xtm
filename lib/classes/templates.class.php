...
class xtm_template
{
    ...
    function load_template($tpl_name)
    {
        ...
        if ($tpl_name == '' || !file_exists($this->dir . "/" . $tpl_name)) {
            die("模板文件不存在，请与管理员确认: " . str_replace(ROOT_DIR, '', $this->dir) . "/" . $tpl_name);
            return false;
        }
    ...
    {
        global $db, $is_logged, $member_id, $cat_info, $config, $user_group, $category_id, $_TIME, $lang, $smartphone_detected, $xtm_module;
    ...
    function check_module($aviable, $block, $action = true)
    {
        ...
        if ($action) {
            if (!(in_array($xtm_module, $aviable)) and ($aviable[0] != "global")) return "";
            else return $block;
        } else {
            if ((in_array($xtm_module, $aviable))) return "";
            else return $block;
        }
    }
