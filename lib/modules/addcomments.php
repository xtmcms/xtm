<?php
if (!defined('XTMCMS') or $config['allow_comments'] != "yes") {
    die("Hacking attempt!");
}
...
if (intval($config['comments_minlen']) AND xtm_strlen(str_replace(" ", "", strip_tags(trim($comments))), $config['charset']) < $config['comments_minlen']) {
...
if (xtm_strlen($name, $config['charset']) > 20) {
...
if (intval($config['auto_wrap'])) {
    if ($config['charset'] == "utf-8") $utf_pref = "u";
    else $utf_pref = "";
...
if ($CN_HALT) {
    ...
    if ($config['allow_combine']) {
        ...
        if ($row['id']) {
            ...
            if (((xtm_strlen($row['text'], $config['charset']) + xtm_strlen($comments, $config['charset'])) > $config['comments_maxlen']) and $update_comments) {
                $update_comments = false;
                $stop[] = $lang['news_err_3'];
                $CN_HALT = true;
                msgbox($lang['all_err_1'], implode("<br />", $stop) . "<br /><br /><a href=\"javascript:history.go(-1)\">" . $lang['all_prev'] . "</a>");
