...
if (!$area) $area = "news";
$allowed_areas = array('news' => array('comments_table' => 'comments', 'counter_table' => 'post'), 'ajax' => array('comments_table' => 'comments', 'counter_table' => 'post'), 'lastcomments' => array('comments_table' => 'comments', 'counter_table' => 'post'),);
...
if ($id and $action == "comm_edit" and $subaction != "addcomment") {
    ...
    if ($have_perm) {
        ...
        } else $text = $parse->decodeBBCodes($row['text'], true, $config['allow_comments_wysiwyg']);
        if ($config['allow_comments_wysiwyg']) {
        ...
        $tpl->copy_template = "<form  method=\"post\" name=\"xtm-comments-form\" id=\"xtm-comments-form\" action=\"\">" . $tpl->copy_template . "
...
<input type=\"hidden\" name=\"id\" value=\"$id\" /></form>";
    ...
    if ($have_perm) {
        ...
        if (xtm_strlen($comments, $config['charset']) > $config['comments_maxlen']) {
            msgbox($lang['comm_err_2'], $lang['news_err_3'] . " <a href=\"javascript:history.go(-1)\">$lang[all_prev]</a>");
        } elseif (intval($config['comments_minlen']) AND xtm_strlen($comments, $config['charset']) < $config['comments_minlen']) {
    ...
    if ($_POST['xtm_allow_hash'] != "" AND $_POST['xtm_allow_hash'] == $xtm_login_hash AND $is_logged AND $user_group[$member_id['user_group']]['del_allc']) {
