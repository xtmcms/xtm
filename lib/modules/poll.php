...
if (!$log['count'] and $user_group[$member_id['user_group']]['allow_poll']) {
    if (!$poll['multiple']) {
        for ($v = 0; $v < sizeof($body); $v++) {
            ...
            $list .= <<<HTML
<div><input name="xtm_poll_votes" type="radio" $sel value="{$v}" /> {$body[$v]}</div>
HTML;
        }
    } else {
        for ($v = 0; $v < sizeof($body); $v++) {
            $list .= <<<HTML
<div><input name="xtm_poll_votes[]" type="checkbox" value="{$v}" /> {$body[$v]}</div>
HTML;
        }
    }
    $allcount = 0;
...
<!--
var xtm_poll_result = 0;
var xtm_poll_voted  = {$allcount};
function doPoll( event ){

    var frm = document.xtmpollform;
	var vote_check = '';
	var news_id = frm.news_id.value;

  if (xtm_poll_voted == 1) { return; }

  if (event != 'results' && xtm_poll_result != 1) {
	...
	if (event == 'vote' && vote_check == '') { xtmalert ('{$lang['poll_failed']}', xtm_info); return; }

	xtm_poll_voted  = 1;

  } else { xtm_poll_result = 1; }

	if (xtm_poll_result == 1 && event == 'vote') { xtm_poll_result = 0; event = 'list'; }
	...
	$.post(xtm_root + "lib/ajax/poll.php", { news_id: news_id, action: event, answer: vote_check, vote_skin: xtm_skin }, function(data){
		...
		$("#xtm-poll-list").fadeOut(500, function() {
			$(this).html(data);
			$(this).fadeIn(500);
		});
...
HTML;
$tpl->copy_template = $ajax_script . "<form  method=\"post\" name=\"xtmpollform\" id=\"xtmpollform\" action=\"\">" . $tpl->copy_template . "<input type=\"hidden\" name=\"news_id\" id=\"news_id\" value=\"" . $row['id'] . "\" /></form>";
$tpl->compile('poll');
$tpl->clear();
?>