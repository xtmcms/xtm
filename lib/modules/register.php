...
function check_reg($name, $email, $password1, $password2, $sec_code = 1, $sec_code_session = 1)
{
    ...
    if (xtm_strlen($name, $config['charset']) > 20) $stop .= $lang['reg_err_3'];
    ...
    if (count($banned_info['email'])) foreach ($banned_info['email'] as $banned) {
        ...
        if ($banned['email'] and preg_match("#^{$banned['email']}$#i", $email)) {
            ...
            } else $lang['reg_err_23'] = str_replace("{descr}", "", $lang['reg_err_23']);
...
if (isset($_POST['submit_reg']) AND !$stopregistration) {
    ...
    if (!$reg_error) {
        ...
        if ($config['registration_type']) {
            ...
            $doaction = "validating";
            $_REQUEST['id'] = rawurlencode(base64_encode($name . "||" . $email . "||" . md5($password1) . "||" . md5(sha1($name . $email . $stronghash . $config['key']))));
        }
    } else {
...
if ($doaction != "validating" AND !$stopregistration) {
    if ($_POST['xtm_rules_accept'] == "yes") {
        $_SESSION['xtm_rules_accept'] = "1";
    }
    if ($config['registration_rules'] and !$_SESSION['xtm_rules_accept']) {
...
if (isset($_POST['submit_val'])) {
    ...
    $pass = md5($user_arr[2]);
    $stronghash = substr(sha1(DBHOST . DBNAME . DBPASS), 5, 25);
    if (md5(sha1($user . $email . $stronghash . $config['key'])) != $user_arr[3]) die('ID not valid!');
    ...
    if (intval($user_group[$member_id['user_group']]['max_info']) > 0 and xtm_strlen($info, $config['charset']) > $user_group[$member_id['user_group']]['max_info']) $stop .= $lang['reg_err_14'];
    if (xtm_strlen($fullname, $config['charset']) > 100) $stop .= $lang['reg_err_15'];
    if (xtm_strlen($land, $config['charset']) > 100) $stop .= $lang['reg_err_16'];
...
if ($doaction == "validating" AND !$stopregistration AND !$_POST['submit_val']) {
    ...
    $stronghash = substr(sha1(DBHOST . DBNAME . DBPASS), 5, 25);
    if (md5(sha1($name . $email . $stronghash . $config['key'])) != $user_arr[3]) die('ID not valid!');
    $name = preg_replace('#\s+#i', ' ', $name);
    ...
    if ($reg_error != "") {
        ...
        if (($_REQUEST['step'] != 2) and $config['registration_type']) {
            ...
            $id = $db->insert_id();
            set_cookie("xtm_user_id", $id, 365);
            set_cookie("xtm_password", $user_arr[2], 365);
            $_SESSION['xtm_user_id'] = $id;
            $_SESSION['xtm_password'] = $user_arr[2];
        }
    }
}
if ($doaction == "validating" and !$stopregistration) {
