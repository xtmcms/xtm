...
if ($config['http_home_url'] == "") {
    $config['http_home_url'] = explode("lib/ajax/newsletter.php", $_SERVER['PHP_SELF']);
...
if ($type == "pm") {
    ...
    if ($empfanger) $result = $db->query("SELECT user_id, name, fullname FROM " . USERPREFIX . "_users WHERE {$empfanger} LIMIT " . $startfrom . "," . $limit);
    else $result = $db->query("SELECT user_id, name, fullname FROM " . USERPREFIX . "_users LIMIT " . $startfrom . "," . $limit);
