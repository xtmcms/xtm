...
if ($config['http_home_url'] == "") {
    $config['http_home_url'] = explode("lib/ajax/search.php", $_SERVER['PHP_SELF']);
...
while ($row = $db->get_row()) {
    ...
    if (xtm_strlen($row['title'], $config['charset']) > 43) $title = xtm_substr($row['title'], 0, 43, $config['charset']) . " ...";
    ...
    if (xtm_strlen($row['short_story'], $config['charset']) > 150) $description = xtm_substr($row['short_story'], 0, 150, $config['charset']) . " ...";
