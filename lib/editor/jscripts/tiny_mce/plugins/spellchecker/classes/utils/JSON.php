...
class Moxiecode_JSONReader
{
    ...
    function _readString($quote)
    {
        ...
        while (($chr = $this->peek()) != -1) {
            switch ($chr) {
                ...
                case '\'':
                case '"':
                    if ($chr == $quote) $endString = true;
                    $chr = $this->read();
                    if ($chr != -1 && $chr != $quote) $output .= $chr;
                    break;
                default:
                    $output .= $this->read();
            }
    ...
    function read($len = 1)
    {
        if ($this->_pos < $this->_len) {
            if ($len > 1) {
                ...
                $this->_pos += $len;
                return $str;
            } else return $this->_data[++$this->_pos];
        }
        return null;
    }
    ...
    function peek()
    {
        if ($this->_pos < $this->_len) return $this->_data[$this->_pos + 1];
        return null;
    }
}
    ...
    function readValue(&$reader)
    {
        ...
        while ($reader->readToken()) {
            switch ($reader->getToken()) {
                ...
                case JSON_START_OBJ:
                case JSON_START_ARRAY:
                    if ($loc == JSON_IN_OBJECT) $this->addArray($key);
                    else $this->addArray(null);
                    $cur = & $obj;
                ...
                switch ($input[$i]) {
                    ...
                    default:
                        ...
                        if (($byte & 0xFE) == 0xFC) {
                            ...
                            $i += 5;
                            $output .= sprintf('\u%04s', bin2hex($this->_utf82utf16($char)));
                        } else if ($byte < 128) $output .= $input[$i];
                }
            }
            return '"' . $output . '"';
        }
    ...
    function _encodeArray($input)
    {
        ...
        for ($i = 0; $i < count($keys); $i++) {
            if (!is_int($keys[$i])) {
                ...
                $isIndexed = false;
            } else $output .= $this->encode($input[$keys[$i]]);
