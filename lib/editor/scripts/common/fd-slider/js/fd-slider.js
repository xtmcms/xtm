...
var fdSlider = (function () {
    ...
    function fdRange(options) {
        ...
        function disableSlider(noCallback) {
            ...
            try {
                setTabIndex(hanxtm, -1);
                removeEvent(hanxtm, "focus", onFocus);
                removeEvent(hanxtm, "blur", onBlur);

                if (!isOpera) {
                    removeEvent(hanxtm, "keydown", onKeyDown);
                    removeEvent(hanxtm, "keypress", onKeyPress);
                } else {
                    removeEvent(hanxtm, "keypress", onKeyDown);
                }
                ;
        ...
        function enableSlider(noCallback) {
            ...
            setTabIndex(hanxtm, 0);
            addEvent(hanxtm, "focus", onFocus);
            addEvent(hanxtm, "blur", onBlur);

            if (!isOpera) {
                addEvent(hanxtm, "keydown", onKeyDown);
                addEvent(hanxtm, "keypress", onKeyPress);
            } else {
                addEvent(hanxtm, "keypress", onKeyDown);
            }
            ;
        ...
        function destroySlider() {
            ...
            wrapper = bar = hanxtm = outerWrapper = timer = null;
        ...
        function redraw() {
            ...
            try {
                var sW = outerWrapper.offsetWidth,
                    ...
                    hW = hanxtm.offsetWidth,
                    hH = hanxtm.offsetHeight,
                    bH = bar.offsetHeight,
                    bW = bar.offsetWidth,
        ...
        function onMouseDown(e) {
            ...
            // Hanxtm mousedown - initiate drag
            if (targ.className.search("fd-slider-hanxtm") != -1) {
                mousePos = vertical ? e.clientY : e.clientX;
                hanxtmPos = parseInt(vertical ? hanxtm.offsetTop : hanxtm.offsetLeft) || 0;
                ...
                posx -= vertical ? y + Math.round(hanxtm.offsetHeight / 2) : x + Math.round(hanxtm.offsetWidth / 2);
        ...
        // Mousemove or touchmove event on the drag hanxtm
        function trackMouse(e) {
            ...
            pixelsToValue(snapToPxValue(hanxtmPos + (vertical ? e.clientY - mousePos : e.clientX - mousePos)));
        ...
        function onTimer() {
            var xtmp = parseInt(vertical ? hanxtm.offsetTop : hanxtm.offsetLeft, 10);
        ...
        function tweenTo(tx) {
            ...
            tweenB = parseInt(vertical ? hanxtm.offsetTop : hanxtm.offsetLeft, 10);
        ...
        // Calculates value according to pixel position of slider hanxtm
        function pixelsToValue(px) {
            ...
            hanxtm.style[vertical ? "top" : "left"] = (px || 0) + "px";
            redrawRange();
        ...
        function valueToPixels(val) {
            ...
            hanxtm.style[vertical ? "top" : "left"] = (scale ? percentToPixels(valueToPercent(value)) : vertical ? Math.round(((max - value) / step) * stepPx) : Math.round(((value - min) / step) * stepPx)) + "px";
            redrawRange();
        ...
        // Calculates a value according to percentage of distance hanxtm has travelled
        ...
        // Calculates the percentage hanxtm position according to form element value
        ...
        function percentToPixels(percent) {
            return ((outerWrapper[vertical ? "offsetHeight" : "offsetWidth"] - hanxtm[vertical ? "offsetHeight" : "offsetWidth"]) / 100) * percent;
        };

        function pixelsToPercent(pixels) {
            return pixels / ((outerWrapper[vertical ? "offsetHeight" : "offsetWidth"] - outerWrapper[hanxtm ? "offsetHeight" : "offsetWidth"]) / 100);
        };
        ...
        function setSliderRange(newMin, newMax) {
            ...
            hanxtm.setAttribute("aria-valuemin", rMin);
            hanxtm.setAttribute("aria-valuemax", rMax);
        ...
        function redrawRange() {
            ...
            ;
            if (vertical) {
                rangeBar.style["height"] = (bar.offsetHeight - hanxtm.offsetTop) + "px";
            } else {
                rangeBar.style["width"] = hanxtm.offsetLeft + "px";
            }
            ;
        };
        ...
        function updateAriaValues() {
            hanxtm.setAttribute("aria-valuenow", tagName == "select" ? inp.options[inp.selectedIndex].value : inp.value);
            hanxtm.setAttribute("aria-valuetext", tagName == "select" ? (inp.options[inp.selectedIndex].text ? inp.options[inp.selectedIndex].text : inp.options[inp.selectedIndex].value) : inp.value);
        };
        ...
        (function () {
            ...
            if (fullARIA) {
                hanxtm = document.createElement('span');
            } else {
                hanxtm = document.createElement('a');
                hanxtm.setAttribute("href", "#");
                addEvent(hanxtm, "click", stopEvent);
            }
            ;

            setTabIndex(hanxtm, 0);

            hanxtm.className = "fd-slider-hanxtm";
            hanxtm.appendChild(document.createTextNode(String.fromCharCode(160)));
            ...
            outerWrapper.appendChild(hanxtm);

            inp.parentNode.insertBefore(outerWrapper, inp);

            /*@cc_on@*/
            /*@if(@_win32)
             hanxtm.unselectable       = "on";
            ...
            hanxtm.setAttribute("role", "slider");
            hanxtm.setAttribute("aria-valuemin", tagName == "select" ? inp.options[0].value : min);
            hanxtm.setAttribute("aria-valuemax", tagName == "select" ? inp.options[inp.options.length - 1].value : max);

            var lbl = findLabel();
            if (lbl) {
                hanxtm.setAttribute("aria-labelledby", lbl.id);
                hanxtm.id = "fd-slider-hanxtm-" + inp.id;
                /*@cc_on
                 /*@if(@_win32)
                 lbl.setAttribute("htmlFor", hanxtm.id);
                 @else @*/
                lbl.setAttribute("for", hanxtm.id);
                /*@end
                 @*/
            }
            ;
            ...
            if (document.getElementById(describedBy)) {
                hanxtm.setAttribute("aria-describedby", describedBy);
            }
            ;
