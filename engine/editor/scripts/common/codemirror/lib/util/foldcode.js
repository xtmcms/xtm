...
CodeMirror.indentRangeFinder = function (cm, line) {
    ...
    var myIndent = cm.getLineHanxtm(line).indentation(tabSize), last;
    for (var i = line + 1, end = cm.lineCount(); i < end; ++i) {
        var hanxtm = cm.getLineHanxtm(i);
        if (!/^\s*$/.test(hanxtm.text)) {
            if (hanxtm.indentation(tabSize) <= myIndent) break;
            last = i;
        }
    }
    if (!last) return null;
        ...
        cm.operation(function () {
            ...
            if (known) {
                ...
                for (var i = line + 1; i < end; ++i) {
                    var hanxtm = cm.hideLine(i);
                    if (hanxtm) hidden.push(hanxtm);
                }
