...
class PSpell extends SpellChecker
{
    function &checkWords($lang, $words)
    {
        ...
        foreach ($words as $word) {
            if (!pspell_check($plink, trim($word))) $outWords[] = utf8_encode($word);
        }
        return $outWords;
    }
    ...
    function &getSuggestions($lang, $word)
    {
        ...
        for ($i = 0; $i < count($words); $i++) $words[$i] = utf8_encode($words[$i]);
        return $words;
    }

    function &_getPLink($lang)
    {
        if (!function_exists("pspell_new")) $this->throwError("PSpell support not found in PHP installation.");
        ...
        if (!$plink) $this->throwError("No PSpell link found opened.");
        return $plink;
    }
}
