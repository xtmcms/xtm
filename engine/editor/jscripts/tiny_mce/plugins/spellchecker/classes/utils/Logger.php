...
class Moxiecode_Logger
{
    ...
    function setMaxSize($size)
    {
        ...
        if (strpos((strtolower($size)), "k") > 0) $logMaxSizeBytes *= 1024;
        if (strpos((strtolower($size)), "m") > 0) $logMaxSizeBytes *= (1024 * 1024);
    ...
    function _logMsg($level, $message)
    {
        ...
        if (file_exists($logFile)) {
            ...
            if ($size + strlen($message) > $this->_maxSizeBytes) $roll = true;
        }
        if ($roll) {
            for ($i = $this->_maxFiles - 1; $i >= 1; $i--) {
                $rfile = $this->toOSPath($logFile . "." . $i);
                $nfile = $this->toOSPath($logFile . "." . ($i + 1));
                if (@file_exists($rfile)) @rename($rfile, $nfile);
            }
