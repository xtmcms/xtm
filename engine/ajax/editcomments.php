...
if ($config['http_home_url'] == "") {
    $config['http_home_url'] = explode("lib/ajax/editcomments.php", $_SERVER['PHP_SELF']);
...
setTimeout(function() {
	...
	$('textarea.ajaxwysiwygeditor').tinymce({
		...
		// Theme options
		theme_advanced_buttons1 : "bold,italic,underline,separator,strikethrough,justifyleft,justifycenter,justifyright, justifyfull,bullist,numlist,separator,{$link_icon}emotions,xtm_quote,xtm_hide",
...
<div class="editor">
{$bb_code}
<textarea name="xtmeditcomments{$id}" id="xtmeditcomments{$id}" onfocus="setNewField(this.name, document.getElementById( 'xtmmasscomments' ) )" class="ajaxwysiwygeditor" style="width:99%; height:150px; border:1px solid #E0E0E0; margin: 0px 1px 0px 0px;padding: 0px;">{$comm_txt}</textarea><br>
...
HTML;
    ...
    if (xtm_strlen($comm_txt, $config['charset']) > $config['comments_maxlen']) {
        die("error");
    }
    if ($comm_txt == "") {
        die("error");
    }
    if (intval($config['comments_minlen']) AND xtm_strlen($comm_txt, $config['charset']) < $config['comments_minlen']) {
        die("error");
    }
    ...
    if (!$row['approve']) {
        if ($config['allow_alt_url'] == "yes" AND !$config['seo_type']) clear_cache('full_');
        else clear_cache('full_' . $row['post_id']);
    }
