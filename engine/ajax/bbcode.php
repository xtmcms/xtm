...
if ($addtype == "addnews") {
    ...
    $startform = "xtmeditnews" . $id;
    $p_name = urlencode($row['autor']);
    if ($is_logged AND $user_group[$member_id['user_group']]['allow_image_upload']) {
        $image_upload = "<div class=\"editor_button\" onclick=\"xtm_image_upload('{$p_name}', '{$row['id']}'); return false;\"><img title=\"$lang[bb_t_up]\" src=\"{THEME}/bbcodes/upload.gif\" width=\"23\" height=\"25\" border=\"0\" alt=\"\" /></div>";
    } else $image_upload = "";
    $code = <<<HTML
...
HTML;
    ...
    $addform = "document.getElementById( 'xtmmasscomments' )";
    $startform = "xtmeditcomments" . $id;
