...
if ($config['http_home_url'] == "") {
    $config['http_home_url'] = explode("lib/ajax/allvotes.php", $_SERVER['PHP_SELF']);
...
$_REQUEST['xtm_skin'] = totranslit($_REQUEST['xtm_skin'], false, false);
if ($_REQUEST['xtm_skin']) {
    if (@is_dir(ROOT_DIR . '/templates/' . $_REQUEST['xtm_skin'])) {
        $config['skin'] = $_REQUEST['xtm_skin'];
    }
}
...
while ($row = $db->get_row($sql_result)) {
    ...
    if ($user_group[$member_id['user_group']]['allow_vote']) {
        ...
        if ($row2['count'] OR count(explode(".", $_IP)) != 4) {
            $disable = $lang['vote_disable_1'];
            $allow_vote = false;
        }
    } else {
        $disable = $lang['vote_not_allow'];
        $allow_vote = false;
    }
    for ($i = 0; $i < sizeof($body); $i++) {
        ...
        if ($allow_vote) $radio = "<input name=\"vote_check\" type=\"radio\" $sel value=\"$i\" />";
        else $radio = "&nbsp;";
    ...
    if ($allow_vote) $button = "<br /><input type=\"submit\" onclick=\"fast_vote('{$row['id']}'); return false;\" class=\"xtmvotebutton\" value=\"{$lang['vote_set']}\" />";
    else $button = "<font color=\"red\">{$disable}</font>";
    $content .= <<<HTML
<form method="post" name="vote_{$row['id']}" id="vote_{$row['id']}" action=''>
<fieldset>
  <legend>{$title}</legend>
  <div id="xtm-vote_list-{$row['id']}">{$entry}{$button}<br /><br />{$lang['max_votes']} {$max}</div>
</fieldset>
</form>
HTML;
}
@header("Content-type: text/html; charset=" . $config['charset']);
echo "<div id=\"xtmvotespopup\" title=\"{$lang['all_votes']}\" style=\"display:none\"><div id=\"xtmvotespopupcontent\" style=\"overflow: auto;\">{$content}</div></div>";
?>