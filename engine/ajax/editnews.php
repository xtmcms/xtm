...
if ($config['http_home_url'] == "") {
    $config['http_home_url'] = explode("lib/ajax/editnews.php", $_SERVER['PHP_SELF']);
...
if ($_COOKIE['xtm_skin']) {
    if (@is_dir(ROOT_DIR . '/templates/' . $_COOKIE['xtm_skin'])) {
...
if ($_REQUEST['action'] == "edit") {
    ...
    foreach ($xfields as $name => $value) {
        $fieldname = $value[0];
        if ($xfieldsdata[$value[0]]) $fieldvalue = $xfieldsdata[$value[0]];
        else continue;
        if ($row['allow_br'] AND !$config['allow_quick_wysiwyg']) {
        ...
        if ($value[3] == "textarea") {
            if (!$config['allow_quick_wysiwyg']) $params = "onfocus=\"setNewField(this.id, document.ajaxnews{$id})\" ";
    ...
    if (!$config['allow_quick_wysiwyg']) {
        ...
        if ($config['allow_quick_wysiwyg'] == "2") {
            if ($user_group[$member_id['user_group']]['allow_image_upload']) $image_upload = "media_upload('short_story', '{$p_name}', '{$row['id']}', '2')";
...
function show_newseditor( root ) {
	...
	oUtil.initializeEditor("wysiwygeditor",  {
		...
		groups:[
			...
			["grpEdit4", "", ["xtmQuote", "xtmCode", "xtmHide", "xtmSpoiler","xtmPasteText", "BRK", "xtmVideo", "xtmAudio", "xtmMedia", "HTML5Video", "xtmTypograf"]],
		...
		arrCustomButtons:[
			...
			["xtmTypograf", "ws_typograf('"+ root +"')", "{$lang['bb_t_t']}", "xtm_tt.gif"],
...
function ws_typograf(root) {
	...
	$.post(root + "lib/ajax/typograf.php", {txt: txt}, function(data){
...
show_newseditor(xtm_root);
</script>
HTML;
        }
        $code = "";
    }
    if (!$config['allow_quick_wysiwyg']) $params = "onfocus=\"setNewField(this.name, document.ajaxnews{$id})\"";
    else $params = "class=\"wysiwygeditor\"";
    $buffer = <<<HTML
...
{$bb_code}
<textarea name="xtmeditnews{$id}" id="xtmeditnews{$id}" {$params} style="width:98%; height:250px; border:1px solid #E0E0E0;margin: 0px 1px 0px 0px;padding: 0px;">{$news_txt}</textarea>
<div><br /><b>{$lang['s_ffull']}</b></div>
{$code}
<textarea name="xtmeditfullnews{$id}" id="xtmeditfullnews{$id}" {$params} style="width:98%; height:250px; border:1px solid #E0E0E0;margin: 0px 1px 0px 0px;padding: 0px;">{$full_txt}</textarea>
{$xfbuffer}
...
HTML;
    ...
    if ($user_group[$member_id['user_group']]['moderation']) $approve = 1;
    else $approve = 0;
    ...
    if ($config['allow_alt_url'] == "yes" AND !$config['seo_type']) $cprefix = "full_";
