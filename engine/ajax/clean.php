...
if ($_REQUEST['step'] == 4) {
    if ((@strtotime($_REQUEST['date']) === -1) OR (@strtotime($_REQUEST['date']) === false) OR (trim($_REQUEST['date']) == "")) $_REQUEST['step'] = 3;
    else {
        $_REQUEST['step'] = 5;
...
if ($_REQUEST['step'] == 2) {
    if ((@strtotime($_REQUEST['date']) === -1) OR (@strtotime($_REQUEST['date']) === false) OR (trim($_REQUEST['date']) == "")) $_REQUEST['step'] = 1;
    else {
        $_REQUEST['step'] = 3;
        $_REQUEST['date'] = $db->safesql($_REQUEST['date']);
        ...
        while ($row = $db->get_row($sql)) {
            ...
            if ($image['images'] != "") foreach ($listimages as $dataimages) {
