...
if (isset($_POST['PHPSESSID']) AND $_REQUEST['subaction'] == "upload") xtm_session($_POST['PHPSESSID']);
else xtm_session();
if ($_COOKIE['xtm_skin']) {
...
if ($_POST['flashmode']) @header("Content-type: text/html; charset=utf-8");
else @header("Content-type: text/html; charset=" . $config['charset']);
if (!$is_logged) {
...
if (isset($_REQUEST['wysiwyg'])) $wysiwyg = totranslit($_REQUEST['wysiwyg'], true, false);
else $wysiwyg = 0;
if (isset($_REQUEST['author'])) $author = @$db->safesql(strip_tags(urldecode($_REQUEST['author'])));
else $author = "";
...
if ($_REQUEST['subaction'] == "upload") {
    ...
    if ($user_group[$member_id['user_group']]['allow_image_size']) {
        if (isset($_REQUEST['t_seite'])) $t_seite = intval($_REQUEST['t_seite']);
        else $t_seite = intval($config['t_seite']);
        if (isset($_REQUEST['make_thumb'])) $make_thumb = intval($_REQUEST['make_thumb']);
        else $make_thumb = true;
        ...
        $make_thumb = true;
        if ($config['allow_watermark'] == "yes") $make_watermark = true;
        else $make_watermark = false;
    }
    ...
    if ($_POST['flashmode']) $flashmode = true;
    else $flashmode = false;
...
if (count($uploaded_list)) $uploaded_list = implode("", $uploaded_list);
else $uploaded_list = "";
if (count($folder_list)) $folder_list = implode("", $folder_list);
else $folder_list = "";
$image_align = array();
...
if ($user_group[$member_id['user_group']]['allow_file_upload']) {
    ...
    elseif ($user_group[$member_id['user_group']]['max_file_size'] > $config['max_up_size']) $max_file_size = (int)$user_group[$member_id['user_group']]['max_file_size']; else $max_file_size = (int)$config['max_up_size'];
} else {
...
HTML;
if ($uploaded_list) $im_show = "tabClick(0);";
else $im_show = "";
echo <<<HTML
