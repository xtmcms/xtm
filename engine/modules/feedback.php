...
if (isset($_POST['send'])) {
    ...
    if ($user_group[$member_id['user_group']]['max_mail_day']) {
        ...
        if (!$is_logged) $check_user = $_IP;
        else $check_user = $db->safesql($member_id['name']);
    ...
    if (empty($name) OR xtm_strlen($name, $config['charset']) > 100) {
        $stop .= $lang['feed_err_1'];
    }
    if (empty($email) OR xtm_strlen($email, $config['charset']) > 50 OR @count(explode("@", $email)) != 2) {
        $stop .= $lang['feed_err_2'];
    }
    if (empty($subject) OR xtm_strlen($subject, $config['charset']) > 200) {
        $stop .= $lang['feed_err_4'];
    }
    if (empty($message) OR xtm_strlen($message, $config['charset']) > 20000) {
...
<script type="text/javascript" src="http://www.google.com/recaptcha/api/challenge?k=' . $config['recaptcha_public_key'] . '"></script>');
                ...
                $tpl->set('[sec_code]', "");
                $tpl->set('[/sec_code]', "");
                $tpl->set('{code}', "<span id=\"xtm-captcha\"><img src=\"" . $path['path'] . "engine/modules/antibot.php\" alt=\"{$lang['sec_image']}\" /><br /><a onclick=\"reload(); return false;\" href=\"#\">{$lang['reload_code']}</a></span>");
...
$(function(){
	...
	$('#sendmail').submit(function() {
		...
		if(document.sendmail.subject.value == '' || document.sendmail.message.value == '') { 
			...
			xtmalert('{$lang['comm_req_f']}', xtm_info);
			return false;
		...
		params['skin'] = xtm_skin;

		ShowLoading('');

		$.post(xtm_root + "engine/ajax/feedback.php", params, function(data){
			HideLoading('');
			if (data) {
	
				if (data.status == "ok") {

					scroll( 0, $("#xtm-content").offset().top - 70 );
					$('#xtm-content').html(data.text);	
