...
function xtm_session($sid = false)
{
...
{
    global $cat_info, $user_group, $member_id, $xtm_module;
    if ($xtm_module == 'addnews') $allow_list = explode(',', $user_group[$member_id['user_group']]['cat_allow_addnews']);
...
function xtm_cache($prefix, $cache_id = false, $member_prefix = false)
{
...
{
    global $db, $is_logged, $member_id, $xf_inited, $cat_info, $config, $user_group, $category_id, $_TIME, $lang, $smartphone_detected, $xtm_module, $allow_comments_ajax, $PHP_SELF;
    ...
    if ($cat_info[$custom_category]['news_sort'] != "") $news_sort = $cat_info[$custom_category]['news_sort'];
    else $news_sort = $config['news_sort'];
    if ($cat_info[$custom_category]['news_msort'] != "") $news_msort = $cat_info[$custom_category]['news_msort'];
    else $news_msort = $config['news_msort'];
    ...
    $content = xtm_cache("news", $custom_cache_id, true);
    if ($content !== false) {
...
function create_keywords($story)
{
    ...
    $metatags['description'] = xtm_substr($story, 0, 190, $config['charset']);
    $story = str_replace($quotes, ' ', $story);
    $arr = explode(" ", $story);
    foreach ($arr as $word) {
        if (xtm_strlen($word, $config['charset']) > 4) $newarr[] = $word;
    }
...
function news_sort($do)
{
    global $config, $lang;
    if (!$do) $do = "main";
    $find_sort = "xtm_sort_" . $do;
    $direction_sort = "xtm_direction_" . $do;
    ...
    if (strtolower($direction) == "asc") {
        $soft_by_array[$soft_by]['image'] = "<img src=\"{THEME}/xtmimages/asc.gif\" alt=\"\" />";
        $soft_by_array[$soft_by]['direction'] = "desc";
    } else {
        $soft_by_array[$soft_by]['image'] = "<img src=\"{THEME}/xtmimages/desc.gif\" alt=\"\" />";
        $soft_by_array[$soft_by]['direction'] = "asc";
    }
    foreach ($soft_by_array as $value) {
        $sort[] = $value['image'] . "<a href=\"#\" onclick=\"xtm_change_sort('{$value['value']}','{$value['direction']}'); return false;\">" . $value['name'] . "</a>";
    }
    $sort = "<form name=\"news_set_sort\" id=\"news_set_sort\" method=\"post\" action=\"\" >" . $lang['sort_main'] . "&nbsp;" . implode(" | ", $sort);
    $sort .= <<<HTML
<input type="hidden" name="xtmnewssortby" id="xtmnewssortby" value="{$config['news_sort']}" />
<input type="hidden" name="xtmdirection" id="xtmdirection" value="{$config['news_msort']}" />
...
function xtm_change_sort(sort, direction){

  var frm = document.getElementById('news_set_sort');

  frm.xtmnewssortby.value=sort;
  frm.xtmdirection.value=direction;
...
function check_static($names, $block, $action = true)
{
    ...
    if ($action) {
        if (in_array($page, $names) AND $xtm_module == "static") {
            $block = str_replace('\"', '"', $block);
            return $block;
        }
    } else {
        if (!in_array($page, $names) OR $xtm_module != "static") {
            $block = str_replace('\"', '"', $block);
            return $block;
        }
    }
...
function xtm_strlen($value, $charset)
{
    if (strtolower($charset) == "utf-8") return iconv_strlen($value, "utf-8");
    else return strlen($value);
}

function xtm_substr($str, $start, $length, $charset)
{
    if (strtolower($charset) == "utf-8") return iconv_substr($str, $start, $length, "utf-8");
    else return substr($str, $start, $length);
}

function xtm_strrpos($str, $neextm, $charset)
{
    if (strtolower($charset) == "utf-8") return iconv_strrpos($str, $neextm, "utf-8");
    else return strrpos($str, $neextm);
}
