...
if ($allow_active_news) {
    ...
    } elseif ($category_id and $cat_info[$category_id]['short_tpl'] != '') $tpl->load_template($cat_info[$category_id]['short_tpl'] . '.tpl'); else $tpl->load_template('shortstory.tpl');
    ...
    if (count($banners) AND $config['allow_banner'] AND !$smartphone_detected) {
        $news_c = 1;
        if (isset($ban_short)) {
            for ($indx = 0, $max = sizeof($ban_short['top']), $banners_topz = ''; $indx < $max; $indx++) if ($ban_short['top'][$indx]['zakr']) {
                $banners_topz .= $ban_short['top'][$indx]['text'];
                unset($ban_short['top'][$indx]);
            }
            for ($indx = 0, $max = sizeof($ban_short['cen']), $banners_cenz = ''; $indx < $max; $indx++) if ($ban_short['cen'][$indx]['zakr']) {
                $banners_cenz .= $ban_short['cen'][$indx]['text'];
                unset($ban_short['cen'][$indx]);
            }
            for ($indx = 0, $max = sizeof($ban_short['down']), $banners_downz = ''; $indx < $max; $indx++) if ($ban_short['down'][$indx]['zakr']) {
    ...
    while ($row = $db->get_row($sql_result)) {
        ...
        if ($is_logged) {
            ...
            if (!in_array($row['id'], $fav_arr) or $config['allow_cache'] == "yes") $tpl->set('{favorites}', "<a id=\"fav-id-" . $row['id'] . "\" href=\"$PHP_SELF?do=favorites&amp;doaction=add&amp;id=" . $row['id'] . "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/xtmimages/plus_fav.gif\" onclick=\"doFavorites('" . $row['id'] . "', 'plus'); return false;\" title=\"" . $lang['news_addfav'] . "\" style=\"vertical-align: middle;border: none;\" alt=\"\" /></a>");
        ...
        if (isset($view_template) AND $view_template == "rss") {
            ...
            if (preg_match("#\\{short-story limit=['\"](.+?)['\"]\\}#i", $tpl->copy_template, $matches)) {
                ...
                if ($count AND xtm_strlen($row['short_story'], $config['charset']) > $count) {
                    $row['short_story'] = xtm_substr($row['short_story'], 0, $count, $config['charset']);
                    if (($temp_dmax = xtm_strrpos($row['short_story'], ' ', $config['charset']))) $row['short_story'] = xtm_substr($row['short_story'], 0, $temp_dmax, $config['charset']);
                }
    ...
    if (!isset($view_template) AND $count_all AND $config['news_navigation'] AND $news_found) {
        ...
        if (isset($cstart) and $cstart != "" and $cstart > 0) {
            ...
            if ($config['allow_alt_url'] == "yes") {
                ...
                if ($prev == 1) $prev_page = $PHP_SELF . "?" . $user_query;
                else $prev_page = $PHP_SELF . "?cstart=" . $prev . "&amp;" . $user_query;
                $tpl->set_block("'\[prev-link\](.*?)\[/prev-link\]'si", "<a href=\"" . $prev_page . "\">\\1</a>");
            }
        } else {
                    ...
                    for ($j = 1; $j <= $enpages_count; $j++) {
                        if ($j != $cstart) {
                            if ($config['allow_alt_url'] == "yes") {
                                ...
                                if ($j == 1) $pages .= "<a href=\"$PHP_SELF?{$user_query}\">$j</a> ";
                                else $pages .= "<a href=\"$PHP_SELF?cstart=$j&amp;$user_query\">$j</a> ";
                            }
                        } else {
                            $pages .= "<span>$j</span> ";
                        }
