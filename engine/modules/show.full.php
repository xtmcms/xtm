...
if ($config['allow_alt_url'] == "yes" AND !$config['seo_type']) $cprefix = "full";
else $cprefix = "full_" . $newsid;
$row = xtm_cache($cprefix, $sql_news);
if ($row) {
    $row = unserialize($row);
    $full_cache = true;
} else {
...
if ($row['id'] AND !$row['approve'] AND $member_id['name'] != $row['autor'] AND $member_id['user_group'] != '1') $perm = 0;
...
if ($row['id'] AND $perm) {
    ...
    elseif ($category_id and $cat_info[$category_id]['full_tpl'] != '') $tpl->load_template($cat_info[$category_id]['full_tpl'] . '.tpl'); else $tpl->load_template('fullstory.tpl');
    ...
    if ($cat_info[$category_id]['icon']) {
        ...
        $tpl->set('{category-icon}', "{THEME}/xtmimages/no_icon.gif");
    }
    if ($category_id) $tpl->set('{category-url}', $config['http_home_url'] . get_url($category_id) . "/");
    ...
    if ($row['allow_comm']) {
        $tpl->set('[com-link]', "<a id=\"xtm-comm-link\" href=\"" . $full_link . "#comment\">");
        $tpl->set('[/com-link]', "</a>");
    ...
    if ($config['related_news'] AND $view_template != "print" AND strpos($tpl->copy_template, "{related-news}") !== false) {
        if ($allow_full_cache) $buffer = xtm_cache("related", $row['id'] . $config['skin'], true);
        else $buffer = false;
        if ($buffer === false) {
            if ($row['related_ids']) {
            ...
            while ($related = $db->get_row()) {
                ...
                if (xtm_strlen($related['title'], $config['charset']) > 75) $related['title'] = xtm_substr($related['title'], 0, 75, $config['charset']) . " ...";
                if ($config['allow_alt_url'] == "yes") {
                    if ($config['seo_type'] == 1 OR $config['seo_type'] == 2) {
                ...
                if (stripos($tpl2->copy_template, "{image-") !== false) {
                    ...
                    $tpl2->copy_template = preg_replace("#\\{image-(.+?)\\}#i", "{THEME}/xtmimages/no_image.jpg", $tpl2->copy_template);
                }
                if (preg_match("#\\{text limit=['\"](.+?)['\"]\\}#i", $tpl2->copy_template, $matches)) {
                    ...
                    if ($count AND xtm_strlen($related['short_story'], $config['charset']) > $count) {
                        $related['short_story'] = xtm_substr($related['short_story'], 0, $count, $config['charset']);
                        if (($temp_dmax = xtm_strrpos($related['short_story'], ' ', $config['charset']))) $related['short_story'] = xtm_substr($related['short_story'], 0, $temp_dmax, $config['charset']);
                    }
                ...
                if ($xfound) {
                    ...
                    foreach ($xfields as $value) {
                        ...
                        if (preg_match("#\\[xfvalue_{$preg_safe_name} limit=['\"](.+?)['\"]\\]#i", $tpl2->copy_template, $matches)) {
                            ...
                            if ($count AND xtm_strlen($xfieldsdata[$value[0]], $config['charset']) > $count) {
                                $xfieldsdata[$value[0]] = xtm_substr($xfieldsdata[$value[0]], 0, $count, $config['charset']);
                                if (($temp_dmax = xtm_strrpos($xfieldsdata[$value[0]], ' ', $config['charset']))) $xfieldsdata[$value[0]] = xtm_substr($xfieldsdata[$value[0]], 0, $temp_dmax, $config['charset']);
                            }
                            $tpl2->set($matches[0], $xfieldsdata[$value[0]]);
                        } else $tpl2->copy_template = str_replace("[xfvalue_{$value[0]}]", $xfieldsdata[$value[0]], $tpl2->copy_template);
                    }
                }
    ...
    if ($is_logged) {
        ...
        if (!in_array($row['id'], $fav_arr)) $tpl->set('{favorites}', "<a id=\"fav-id-" . $row['id'] . "\" href=\"$PHP_SELF?do=favorites&amp;doaction=add&amp;id=" . $row['id'] . "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/xtmimages/plus_fav.gif\" onclick=\"doFavorites('" . $row['id'] . "', 'plus'); return false;\" title=\"" . $lang['news_addfav'] . "\" style=\"vertical-align: middle;border: none;\" alt=\"\" /></a>");
        else $tpl->set('{favorites}', "<a id=\"fav-id-" . $row['id'] . "\" href=\"$PHP_SELF?do=favorites&amp;doaction=del&amp;id=" . $row['id'] . "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/xtmimages/minus_fav.gif\" onclick=\"doFavorites('" . $row['id'] . "', 'minus'); return false;\" title=\"" . $lang['news_minfav'] . "\" style=\"vertical-align: middle;border: none;\" alt=\"\" /></a>");
    ...
    if (preg_match("#\\{title limit=['\"](.+?)['\"]\\}#i", $tpl->copy_template, $matches)) {
        ...
        if ($count AND xtm_strlen($row['title'], $config['charset']) > $count) {
            $row['title'] = xtm_substr($row['title'], 0, $count, $config['charset']);
            if (($temp_dmax = xtm_strrpos($row['title'], ' ', $config['charset']))) $row['title'] = xtm_substr($row['title'], 0, $temp_dmax, $config['charset']);
        }
    ...
    if (stripos($tpl->copy_template, "{image-") !== false) {
        ...
        $tpl->copy_template = preg_replace("#\\{image-(.+?)\\}#i", "{THEME}/xtmimages/no_image.jpg", $tpl->copy_template);
    }
    if ($smartphone_detected) {
        if (!$config['allow_smart_format']) {
            ...
            if (!$config['allow_smart_video']) {
                $row['short_story'] = preg_replace("#<!--xtm_video_begin(.+?)<!--xtm_video_end-->#is", "", $row['short_story']);
                $row['short_story'] = preg_replace("#<!--xtm_audio_begin(.+?)<!--xtm_audio_end-->#is", "", $row['short_story']);
                $row['full_story'] = preg_replace("#<!--xtm_video_begin(.+?)<!--xtm_video_end-->#is", "", $row['full_story']);
                $row['full_story'] = preg_replace("#<!--xtm_audio_begin(.+?)<!--xtm_audio_end-->#is", "", $row['full_story']);
            }
        }
    }
    $tpl->set('{comments}', "<!--xtmcomments-->");
    $tpl->set('{addcomments}', "<!--xtmaddcomments-->");
    $tpl->set('{navigation}', "<!--xtmnavigationcomments-->");
    ...
    if (preg_match("#\\{full-story limit=['\"](.+?)['\"]\\}#i", $tpl->copy_template, $matches)) {
        ...
        if ($count AND xtm_strlen($row['full_story'], $config['charset']) > $count) {
            $row['full_story'] = xtm_substr($row['full_story'], 0, $count, $config['charset']);
            if (($temp_dmax = xtm_strrpos($row['full_story'], ' ', $config['charset']))) $row['full_story'] = xtm_substr($row['full_story'], 0, $temp_dmax, $config['charset']);
        }
...
if ($allow_comments AND $news_found) {
    if ($comments_num > 0) {
        ...
        $comments = new xtm_Comments($db, $comments_num, intval($config['comm_nummers']));
        ...
        if ($allow_full_cache AND $config['allow_comments_cache']) $allow_full_cache = $news_id;
        else $allow_full_cache = false;
        $comments->build_comments('comments.tpl', 'news', $allow_full_cache, $full_link);
        unset ($tpl->result['comments']);
        if (isset($_GET['news_page']) AND $_GET['news_page']) $user_query = "newsid=" . $newsid . "&amp;news_page=" . intval($_GET['news_page']);
        else $user_query = "newsid=" . $newsid;
    ...
    if ($user_group[$member_id['user_group']]['allow_addc'] AND $config['allow_comments'] == "yes" AND $allow_add AND ($member_id['restricted'] != 2 AND $member_id['restricted'] != 3)) {
        if (!$comments_num) {
            if (strpos($tpl->result['content'], "<!--xtmcomments-->") !== false) {
                $tpl->result['content'] = str_replace("<!--xtmcomments-->", "\n<div id=\"xtm-ajax-comments\"></div>\n", $tpl->result['content']);
            } else $tpl->result['content'] .= "\n<div id=\"xtm-ajax-comments\"></div>\n";
        }
        $tpl->load_template('addcomments.tpl');
        if ($config['allow_subscribe'] AND $user_group[$member_id['user_group']]['allow_subscribe']) $allow_subscribe = true;
        else $allow_subscribe = false;
        ...
        if ($user_group[$member_id['user_group']]['comments_question']) {
            ...
            $tpl->set('{question}', "<span id=\"xtm-question\">" . htmlspecialchars(stripslashes($question['question']), ENT_QUOTES, $config['charset']) . "</span>");
        ...
        if ($user_group[$member_id['user_group']]['captcha']) {
            if ($config['allow_recaptcha']) {
                $tpl->set('[recaptcha]', "");
                $tpl->set('[/recaptcha]', "");
                $tpl->set('{recaptcha}', '<div id="xtm_recaptcha"></div>');
        ...
        $tpl->copy_template = "<form  method=\"post\" name=\"xtm-comments-form\" id=\"xtm-comments-form\" action=\"{$_SESSION['referrer']}\">" . $tpl->copy_template . "
...
$(function(){
	...
	$('#xtm-comments-form').submit(function() {
	  doAddComments();
	  return false;
	});
...
HTML;
        ...
        $tpl->clear();
        if (strpos($tpl->result['content'], "<!--xtmaddcomments-->") !== false) {
            $tpl->result['content'] = str_replace("<!--xtmaddcomments-->", $tpl->result['addcomments'], $tpl->result['content']);
        } else {
