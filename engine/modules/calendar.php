...
if ($config['allow_calendar'] == "yes") {
    ...
    $tpl->result['calendar'] = xtm_cache("calendar", $cache_id);
...
if ($config['allow_archives'] == "yes") {
    $tpl->result['archive'] = xtm_cache("archives", $config['skin']);
    if (!$tpl->result['archive']) {
        ...
        if ($i > 6) {
            $news_archive[6] = "<div id=\"xtm_news_archive\" style=\"display:none;\">" . $news_archive[6];
            $news_archive[] = "</div><div id=\"xtm_news_archive_link\" ><br /><a class=\"archives\" onclick=\"$('#xtm_news_archive').toggle('blind',{},700); return false;\" href=\"#\">" . $lang['show_archive'] . "</a></div>";
        }
