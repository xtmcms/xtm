...
if (isset($_REQUEST['action']) and $_REQUEST['action'] == "logout") {
    $xtm_user_id = "";
    $xtm_password = "";
    set_cookie("xtm_user_id", "", 0);
    set_cookie("xtm_name", "", 0);
    set_cookie("xtm_password", "", 0);
    set_cookie("xtm_skin", "", 0);
    set_cookie("xtm_newpm", "", 0);
    set_cookie("xtm_hash", "", 0);
    ...
    if ($allow_login AND $allow_user) {
        ...
        if ($member_id['user_id']) {
            if (intval($_POST['login_not_save'])) {
                ...
                set_cookie("xtm_user_id", $member_id['user_id'], 365);
                set_cookie("xtm_password", $_POST['login_password'], 365);
            }
            $_SESSION['xtm_user_id'] = $member_id['user_id'];
            $_SESSION['xtm_password'] = $_POST['login_password'];
            $_SESSION['member_lasttime'] = $member_id['lastdate'];
            $member_id['lastdate'] = $_TIME;
            $xtm_login_hash = md5($_SERVER['HTTP_HOST'] . $member_id['user_id'] . sha1($_POST['login_password']) . $config['key'] . date("Ymd"));
            if ($config['log_hash']) {
                ...
                set_cookie("xtm_hash", $hash, 365);
                $_COOKIE['xtm_hash'] = $hash;
                $member_id['hash'] = $hash;
            } else $db->query("UPDATE LOW_PRIORITY " . USERPREFIX . "_users set lastdate='{$_TIME}', logged_ip='" . $_IP . "' WHERE user_id='{$member_id['user_id']}'");
            $is_logged = true;
        }
    }
} elseif (isset($_SESSION['xtm_user_id']) AND intval($_SESSION['xtm_user_id']) > 0 AND $_SESSION['xtm_password']) {
    $member_id = $db->super_query("SELECT * FROM " . USERPREFIX . "_users WHERE user_id='" . intval($_SESSION['xtm_user_id']) . "'");
    if ($member_id['user_id'] AND $member_id['password'] AND $member_id['password'] == md5($_SESSION['xtm_password'])) {
        $is_logged = true;
        $xtm_login_hash = md5($_SERVER['HTTP_HOST'] . $member_id['user_id'] . sha1($_SESSION['xtm_password']) . $config['key'] . date("Ymd"));
    } else {
        $member_id = array();
        $is_logged = false;
...
} elseif (isset($_COOKIE['xtm_user_id']) AND intval($_COOKIE['xtm_user_id']) > 0 AND $_COOKIE['xtm_password']) {
    if ($config['login_log']) $allow_login = check_allow_login($_IP, $config['login_log']);
    if ($allow_login) {
        $member_id = $db->super_query("SELECT * FROM " . USERPREFIX . "_users WHERE user_id='" . intval($_COOKIE['xtm_user_id']) . "'");
        if ($member_id['user_id'] AND $member_id['password'] AND $member_id['password'] == md5($_COOKIE['xtm_password'])) {
            $is_logged = true;
            $xtm_login_hash = md5($_SERVER['HTTP_HOST'] . $member_id['user_id'] . sha1($_COOKIE['xtm_password']) . $config['key'] . date("Ymd"));
            $_SESSION['xtm_user_id'] = $member_id['user_id'];
            $_SESSION['xtm_password'] = $_COOKIE['xtm_password'];
            $_SESSION['member_lasttime'] = $member_id['lastdate'];
        } else {
            $member_id = array();
            $is_logged = false;
        ...
        if ($config['log_hash'] and (($_COOKIE['xtm_hash'] != $member_id['hash']) or ($member_id['hash'] == ""))) {
            $member_id = array();
            $is_logged = false;
        }
    }
...
if (!$is_logged) {
    ...
    set_cookie("xtm_password", "", 0);
    set_cookie("xtm_hash", "", 0);
    $_SESSION['xtm_user_id'] = 0;
    $_SESSION['xtm_password'] = "";
}
?>