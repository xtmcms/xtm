...
if ($allow_userinfo and $doaction == "adduserinfo") {
    $stop = false;
    $id = intval($_POST['id']);
    if (!$is_logged OR $_POST['xtm_allow_hash'] == "" OR $_POST['xtm_allow_hash'] != $xtm_login_hash OR !$id) {
    ...
    if (!$is_logged or !($member_id['user_id'] == $row['user_id'] or $member_id['user_group'] == 1)) {
        ...
        if ($_POST['allow_mail']) $allow_mail = 0;
        else $allow_mail = 1;
        ...
        if ($_POST['allowed_ip']) {
            ...
            if (count($allowed_ip)) $allowed_ip = $db->safesql($parse->process(implode("|", $allowed_ip)));
            else $allowed_ip = "";
        } else $allowed_ip = "";
        ...
        if ($config['registration_type'] AND $email != $row['email'] AND !$user_group[$member_id['user_group']]['admin_editusers']) $send_mail_log = true;
        else $send_mail_log = false;
        if (intval($user_group[$member_id['user_group']]['max_info']) > 0 and xtm_strlen($info, $config['charset']) > $user_group[$member_id['user_group']]['max_info']) {
            $stop .= $lang['news_err_22'];
        }
        if (intval($user_group[$member_id['user_group']]['max_signature']) > 0 and xtm_strlen($signature, $config['charset']) > $user_group[$member_id['user_group']]['max_signature']) {
            $stop .= $lang['not_allowed_sig'];
        }
        if (xtm_strlen($fullname, $config['charset']) > 100) {
            $stop .= $lang['news_err_23'];
        }
        if (preg_match("/[\||\'|\<|\>|\"|\!|\]|\?|\$|\@|\/|\\\|\&\~\*\+]/", $fullname)) {
            $stop .= $lang['news_err_35'];
        }
        if (xtm_strlen($land, $config['charset']) > 100) {
...
while ($row = $db->get_row($sql_result)) {
    ...
    if ($is_logged and ($member_id['user_id'] == $row['user_id'] OR $member_id['user_group'] == 1)) {
        ...
        while ($temp_row = $db->get_row($temp_result)) {
            ...
            $ignore_list[] = "<span id=\"xtm-ignore-list-{$temp_row['id']}\">{$user_name}&nbsp;<a title=\"{$lang['del_from_ignore_1']}\" href=\"javascript:DelIgnorePM('" . $temp_row['id'] . "', '" . $lang['del_from_ignore'] . "')\"><img style=\"vertical-align: middle;border:none;\" alt=\"\" src=\"{THEME}/xtmimages/delete.png\" /></a>";
        }
        $db->free($temp_result);
        if (count($ignore_list)) $tpl->set('{ignore-list}', implode(",&nbsp;</span>", $ignore_list) . "</span>");
        else $tpl->set('{ignore-list}', "");
    } else {
        $tpl->set('{edituser}', "");
        $tpl->set('{ignore-list}', "");
