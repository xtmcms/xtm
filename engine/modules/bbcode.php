...
if (isset($addtype) AND $addtype == "addnews") {
    ...
    if ($is_logged AND $user_group[$member_id['user_group']]['allow_image_upload'] OR ($is_logged AND $member_id['user_group'] == 1)) {
        $image_upload = "<div class=\"editor_button\" onclick=\"xtm_image_upload( '{$p_name}', '{$add_id}' ); return false;\"><img title=\"$lang[bb_t_up]\" src=\"{THEME}/bbcodes/upload.gif\" width=\"23\" height=\"25\" border=\"0\" alt=\"\" /></div>";
    } else {
        $image_upload = "";
    }
    $code = <<<HTML
...
<div id="xtm_emos" style="display: none;" title="{$lang['bb_t_emo']}"><div style="width:100%;height:100%;overflow: auto;">{$output}</div></div>
HTML;
} else {
    $startform = "comments";
    $addform = "document.getElementById( 'xtm-comments-form' )";
    $add_id = false;
...
<div id="xtm_emos" style="display: none;" title="{$lang['bb_t_emo']}"><div style="width:100%;height:100%;overflow: auto;">{$output}</div></div>
<textarea name="comments" id="comments" cols="" rows="" style="width:465px;height:156px;border:0px;margin: 0px 1px 0px 0px;padding: 0px;" onfocus="setNewField(this.name, document.getElementById( 'xtm-comments-form' ))">{text}</textarea>
</div>
HTML;
}
