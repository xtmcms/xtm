...
$flag = 0;
$data = array();
if (isset($_REQUEST['vote_action'])) $vote_action = $_REQUEST['vote_action'];
else $vote_action = "";
if (isset($_REQUEST['vote_id'])) $vote_id = intval($_REQUEST['vote_id']);
else $vote_id = 0;
if (isset($_REQUEST['vote_check'])) $vote_check = intval($_REQUEST['vote_check']);
else $vote_check = 0;
...
if (!$user_group[$member_id['user_group']]['allow_vote']) $not_allow_vote = "if (event == 'vote') { xtmalert('{$lang['vote_not_allow']}', xtm_info); return false;}";
else $not_allow_vote = "";
$ajax_script = <<<HTML
...
function doVote( event ){
	...
	{$not_allow_vote}
	var vote_check = $('#xtm-vote input:radio[name=vote_check]:checked').val();
...
switch ($flag) {
    case 0 :
        ...
        $entry = "<div id=\"xtm-vote\">$entry</div>";
