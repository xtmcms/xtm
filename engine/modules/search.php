...
if (!$user_group[$member_id['user_group']]['allow_search']) {
    ...
    if ($config['no_date'] AND !$config['news_future']) $this_date = " AND " . PREFIX . "_post.date < '" . $this_date . "'";
    else $this_date = "";
    if (isset($_REQUEST['story'])) $story = xtm_substr(strip_data(rawurldecode($_REQUEST['story'])), 0, 90, $config['charset']);
    else $story = "";
    if (isset($_REQUEST['search_start'])) $search_start = intval($_REQUEST['search_start']);
    else $search_start = 0;
    if (isset($_REQUEST['titleonly'])) $titleonly = intval($_REQUEST['titleonly']);
    else $titleonly = 0;
    if (isset($_REQUEST['searchuser'])) $searchuser = xtm_substr($_REQUEST['searchuser'], 0, 40, $config['charset']);
    else $searchuser = "";
    if (isset($_REQUEST['exactname'])) $exactname = $_REQUEST['exactname'];
    else $exactname = "";
    if (isset($_REQUEST['all_word_seach'])) $all_word_seach = intval($_REQUEST['all_word_seach']);
    else $all_word_seach = 0;
    if (isset($_REQUEST['replyless'])) $replyless = intval($_REQUEST['replyless']);
    else $replyless = 0;
    if (isset($_REQUEST['replylimit'])) $replylimit = intval($_REQUEST['replylimit']);
    else $replylimit = 0;
    if (isset($_REQUEST['searchdate'])) $searchdate = intval($_REQUEST['searchdate']);
    else $searchdate = 0;
    if (isset($_REQUEST['beforeafter'])) $beforeafter = htmlspecialchars($_REQUEST['beforeafter'], ENT_QUOTES, $config['charset']);
    else $beforeafter = "after";
    if (preg_match("/[\||\'|\<|\>|\"|\!|\?|\$|\@|\/|\\\|\&\~\*\+]/", $searchuser)) $searchuser = "";
    if ($config['full_search']) {
        if (isset($_REQUEST['sortby'])) $sortby = htmlspecialchars($_REQUEST['sortby'], ENT_QUOTES, $config['charset']);
        else $sortby = "";
    } else {
        if (isset($_REQUEST['sortby'])) $sortby = htmlspecialchars($_REQUEST['sortby'], ENT_QUOTES, $config['charset']);
        else $sortby = "date";
    }
    if (isset($_REQUEST['resorder'])) $resorder = htmlspecialchars($_REQUEST['resorder'], ENT_QUOTES, $config['charset']);
    else $resorder = "desc";
    if (isset($_REQUEST['showposts'])) $showposts = intval($_REQUEST['showposts']);
    else $showposts = 0;
    if (isset($_REQUEST['result_from'])) $result_from = intval($_REQUEST['result_from']);
    else $result_from = 1;
...
HTML;
    ...
    if ($subaction == "search") {
        if ($config['full_search']) {
            ...
            $story = array();
            foreach ($arr as $word) {
                $worxtmn = xtm_strlen(trim($word), $config['charset']);
                if ($worxtmn >= $config['search_length_min']) $story[] = $word;
                if ($worxtmn > $story_maxlen) {
                    $story_maxlen = $worxtmn;
                }
            }
            $story = implode(" ", $story);
        } else {
            if (!$all_word_seach) $story = preg_replace("#(\s+|__OR__)#i", '%', $story);
            $story_maxlen = xtm_strlen(trim($story), $config['charset']);
        }
        if ((empty($story) or ($story_maxlen < $config['search_length_min'])) and (empty($searchuser) or (strlen($searchuser) < $config['search_length_min']))) {
            ...
            if ($config['full_search']) {
                $titleonly_where = array('0' => "MATCH(title,short_story,full_story,xfields) AGAINST ('{story}')", '1' => "MATCH(text) AGAINST ('{story}')", '2' => "MATCH(" . PREFIX . "_static.template) AGAINST ('{story}')", '3' => "title LIKE '%{story}%'");
            } else {
                $titleonly_where = array('0' => "short_story LIKE '%{story}%' OR full_story LIKE '%{story}%' OR xfields LIKE '%{story}%' OR title LIKE '%{story}%'", '1' => "text LIKE '%{story}%'", '2' => PREFIX . "_static.template LIKE '%{story}%'", '3' => "title LIKE '%{story}%'");
            }
            if (!empty($story)) {
                    ...
                    if ($titleonly == 2) {
                        ...
                        if ($row['grouplevel'][0] != "all" and !in_array($member_id['user_group'], $row['grouplevel'])) {
                            ...
                            if ($row['allow_template']) {
                                ...
                                if (xtm_strlen($row['story'], $config['charset']) > 2000) {
                                    $row['story'] = xtm_substr(strip_tags($row['story']), 0, 2000, $config['charset']) . " .... ";
                        ...
                        if ($row['output_comms'] == '1') {
                            ...
                            if ($is_logged AND $edit_limit AND (($member_id['name'] == $row['name'] and $row['is_register'] and $user_group[$member_id['user_group']]['allow_delc']) or $member_id['user_group'] == '1' or $user_group[$member_id['user_group']]['del_allc'])) {
                                $tpl->set('[com-del]', "<a href=\"javascript:DeleteComments('{$row['coms_id']}', '{$xtm_login_hash}')\">");
                                $tpl->set('[/com-del]', "</a>");
                            ...
                            if ($row['banned'] == 'yes' or $row['name'] == '' or !$row['is_register']) {
                                $tpl->set('{foto}', "{THEME}/xtmimages/noavatar.png");
                            } else {
                                if ($row['foto']) $tpl->set('{foto}', $config['http_home_url'] . "uploads/fotos/" . $row['foto']);
                                else $tpl->set('{foto}', "{THEME}/xtmimages/noavatar.png");
                            }
                            ...
                            if ($smartphone_detected) {
                                if (!$config['allow_smart_format']) {
                                    ...
                                    if (!$config['allow_smart_video']) {
                                        $row['story'] = preg_replace("#<!--xtm_video_begin(.+?)<!--xtm_video_end-->#is", "", $row['story']);
                                        $row['story'] = preg_replace("#<!--xtm_audio_begin(.+?)<!--xtm_audio_end-->#is", "", $row['story']);
                                    }
                                }
                            }
                            if ($is_logged) {
                                $fav_arr = explode(',', $member_id['favorites']);
                                if (!in_array($row['id'], $fav_arr)) $tpl->set('{favorites}', "<a id=\"fav-id-" . $row['id'] . "\" href=\"$PHP_SELF?do=favorites&amp;doaction=add&amp;id=" . $row['id'] . "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/xtmimages/plus_fav.gif\" onclick=\"doFavorites('" . $row['id'] . "', 'plus'); return false;\" alt=\"" . $lang['news_addfav'] . "\" align=\"middle\" border=\"0\" /></a>");
                                else $tpl->set('{favorites}', "<a id=\"fav-id-" . $row['id'] . "\" href=\"$PHP_SELF?do=favorites&amp;doaction=del&amp;id=" . $row['id'] . "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/xtmimages/minus_fav.gif\" onclick=\"doFavorites('" . $row['id'] . "', 'minus'); return false;\" alt=\"" . $lang['news_minfav'] . "\" align=\"middle\" border=\"0\" /></a>");
                            } else $tpl->set('{favorites}', "");
                            $tpl->set('{result-text}', "<div id='news-id-" . $row['id'] . "' style=\"display:inline;\">" . $row['story'] . "</div>");
                        }
                        ...
                        if ($row['category']) $tpl->set('{category-url}', $config['http_home_url'] . get_url($row['category']) . "/");
