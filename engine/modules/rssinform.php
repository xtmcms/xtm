...
if (count($informers)) {
    ...
    foreach ($temp as $key => $value) {
        ...
        $buffer = xtm_cache("informer_" . $value['id'], $config['skin']);
        if ($buffer) {
        ...
        if (!$buffer) {
            ...
            foreach ($xml->content as $content) {
                ...
                if ($value['tmax'] and xtm_strlen($content['title'], $config['charset']) > $value['tmax']) $content['title'] = xtm_substr($content['title'], 0, $value['tmax'], $config['charset']) . " ...";
                ...
                if ($value['dmax'] and xtm_strlen($content['description'], $config['charset']) > $value['dmax']) {
                    $content['description'] = xtm_substr($content['description'], 0, $value['dmax'], $config['charset']);
                    if (($temp_dmax = xtm_strrpos($content['description'], ' ', $config['charset']))) $content['description'] = xtm_substr($content['description'], 0, $temp_dmax, $config['charset']);
                    $content['description'] .= " ...";
                }
