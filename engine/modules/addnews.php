...
if (!$allow_addnews) {
    ...
    if (isset($_REQUEST['mod']) and $_REQUEST['mod'] == "addnews" and $is_logged and $user_group[$member_id['user_group']]['allow_adds']) {
        ...
        elseif (@preg_match("/[\||\<|\>|\"|\!|\?|\$|\@|\/|\\\|\&\~\*\+]/", $_POST['tags'])) $_POST['tags'] = ""; else $_POST['tags'] = @$db->safesql(htmlspecialchars(strip_tags(stripslashes(trim($_POST['tags']))), ENT_COMPAT, $config['charset']));
        if ($_POST['tags']) {
            $temp_array = array();
            $tags_array = array();
        ...
        if (xtm_strlen($title, $config['charset']) > 200) $stop .= $lang['add_err_2'];
        if ($config['create_catalog']) $catalog_url = $db->safesql(xtm_substr(htmlspecialchars(strip_tags(stripslashes(trim($title))), ENT_QUOTES, $config['charset']), 0, 1, $config['charset']));
        else $catalog_url = "";
	...
	function checkxf ( )
	{
		...
		$('[uid=\"essential\"]:visible').each(function(indx) {
			...
			if($.trim($(this).find('[rel=\"essential\"]').val()).length < 1) {
				...
				xtmalert('{$lang['addnews_xf_alert']}', xtm_info);
		...
		if(document.entryform.title.value == ''){
			...
			xtmalert('{$lang['add_err_7']}', xtm_info); 
