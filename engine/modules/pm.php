...
if ($doaction == "del" AND !$stop_pm AND count($_POST['selected_pm'])) {
    if ($_REQUEST['xtm_allow_hash'] == "" or $_REQUEST['xtm_allow_hash'] != $xtm_login_hash) {
...
$tpl->copy_template = "
    ...
    function confirmDelete(url){
	    xtmconfirm( '{$lang['pm_confirm']}', xtm_confirm, function () {
...
if (isset($_POST['send']) and !$stop_pm) {
    ...
    if (xtm_strlen($_POST['comments'], $config['charset']) > 65000) $_POST['comments'] = "";
    $stop = "";
    if ($config['sec_addnews']) {
        $id_key = $_POST[$_SESSION['id_key']];
        if ($id_key == "" or $id_key != $xtm_login_hash) $stop .= "<li>ANTISPAM: User ID not valid</li>";
    ...
    if (xtm_strlen($subj, $config['charset']) > 250) {
        $stop .= $lang['pm_err_3'];
    }
    ...
    if (!$stop) {
        ...
        $stop_pm = true;
    } else msgbox($lang['all_err_1'], "<ul>" . $stop . "</ul>");
}
if ($doaction == "del" AND !$stop_pm AND $_GET['pmid']) {
    ...
    if ($db->num_rows() < 1) {
        ...
        if ($row['user'] == $member_id['user_id'] AND !$row['pm_read'] AND $row['folder'] == "inbox") {
            $db->query("UPDATE " . USERPREFIX . "_users SET pm_unread=pm_unread-1  WHERE user_id='{$member_id['user_id']}'");
            if ($row['sendid']) $addwhere = " OR id='{$row['sendid']}'";
            else $addwhere = "";
        ...
        $tpl->set('[/reply]', "</a>");
        $tpl->set('[del]', "<a href=\"javascript:confirmDelete('" . $config['http_home_url'] . "index.php?do=pm&amp;doaction=del&amp;pmid=" . $row['id'] . "&amp;xtm_allow_hash=" . $xtm_login_hash . "')\">");
        $tpl->set('[/del]', "</a>");
    ...
    $ajax_form = <<<HTML
<span id="xtm-pm-preview"></span>
<script language="javascript" type="text/javascript">
<!--
function xtmPMPreview( ){ 
	...
	if (xtm_wysiwyg == "2") {
		...
		var pm_text = document.getElementById('xtm-comments-form').comments.value;

	}

	if(document.getElementById('xtm-comments-form').name.value == '' || document.getElementById('xtm-comments-form').subj.value == '' || pm_text == '')
	{
		xtmalert('{$lang['comm_req_f']}', xtm_info);return false;

	}

	var name = document.getElementById('xtm-comments-form').name.value;
	var subj = document.getElementById('xtm-comments-form').subj.value;
	...
	$.post(xtm_root + "lib/ajax/pm.php", { text: pm_text, name: name, subj: subj, skin: xtm_skin }, function(data){
		...
		$("#xtm-pm-preview").html(data);

		$("html"+( ! $.browser.opera ? ",body" : "")).animate({scrollTop: $("#xtm-pm-preview").position().top - 70}, 1100);
...
<script type="text/javascript" src="http://www.google.com/recaptcha/api/challenge?k=' . $config['recaptcha_public_key'] . '"></script>');
    ...
    if ($replyid) {
        ...
        if (strpos($row['subj'], "RE:") === false) $tpl->set('{subj}', "RE: " . stripslashes($row['subj']));
        else $tpl->set('{subj}', stripslashes($row['subj']));
    ...
    if ($config['sec_addnews']) {
        ...
        $random_key = "<input name=\"{$random_key}\" type=\"hidden\" value=\"{$xtm_login_hash}\" />";
    } else $random_key = "";
    if ($config['allow_comments_wysiwyg'] == "2") $tpl->copy_template = "<form  method=\"post\" name=\"xtm-comments-form\" id=\"xtm-comments-form\" onsubmit=\"document.getElementById('comments').value = tinyMCE.get('comments').getContent(); if(document.getElementById('xtm-comments-form').name.value == '' || document.getElementById('xtm-comments-form').subj.value == '' || document.getElementById('comments').value == ''){xtmalert('{$lang['comm_req_f']}', xtm_info);return false}\" action=\"\">\n" . $tpl->copy_template . "{$random_key}<input name=\"send\" type=\"hidden\" value=\"send\" /></form>";
    else $tpl->copy_template = "<form  method=\"post\" name=\"xtm-comments-form\" id=\"xtm-comments-form\" onsubmit=\"if(document.getElementById('xtm-comments-form').name.value == '' || document.getElementById('xtm-comments-form').subj.value == '' || document.getElementById('comments').value == ''){xtmalert('{$lang['comm_req_f']}', xtm_info);return false}\" action=\"\">\n" . $tpl->copy_template . "{$random_key}<input name=\"send\" type=\"hidden\" value=\"send\" /></form>";
    if ($row['user_id']) {
        $db->query("SELECT id FROM " . USERPREFIX . "_ignore_list WHERE user='{$row['user_id']}' AND user_from='{$member_id['name']}'");
        if ($db->num_rows()) {
            $stop_pm = true;
...
<input type="hidden" name="xtm_allow_hash" value="{$xtm_login_hash}" />
HTML;
    ...
    while ($row = $db->get_row()) {
        ...
        if ($row['pm_read']) {
            ...
            $icon = "{THEME}/xtmimages/read.gif";
        } else {
            $subj = "<a class=\"pm_list\" href=\"$PHP_SELF?do=pm&amp;doaction=readpm&amp;pmid=" . $row['id'] . "\"><b>" . stripslashes($row['subj']) . "</b></a>";
            $icon = "{THEME}/xtmimages/unread.gif";
        }
        if ($row['reply']) $icon = "{THEME}/xtmimages/send.gif";
    ...
    if ($count_all AND $count_all > $pm_per_page) {
        if (isset($cstart) and $cstart > 0) {
            ...
            if ($prev == 1) $pages .= "<a href=\"$PHP_SELF?{$user_query}\"> << </a> ";
            else $pages .= "<a href=\"$PHP_SELF?cstart=$prev&amp;$user_query\"> << </a> ";
        }
