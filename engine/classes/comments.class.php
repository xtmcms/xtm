...
class xtm_Comments
{
    ...
    function xtm_Comments($db, $total_comments, $comments_per_pages)
    {
        $this->db = $db;
    ...
    {
        global $config, $tpl, $is_logged, $member_id, $user_group, $lang, $xtm_login_hash, $_TIME, $allow_comments_ajax, $ajax_adds;
        ...
        if ($area != 'ajax' AND $config['comm_msort'] == "DESC") $tpl->copy_template = "\n<div id=\"xtm-ajax-comments\"></div>\n" . $tpl->copy_template;
        if ($area != 'ajax') $tpl->copy_template = "<form method=\"post\" action=\"\" name=\"xtmmasscomments\" id=\"xtmmasscomments\"><div id=\"xtm-comments-list\">\n" . $tpl->copy_template;
        if ($area != 'ajax') $tpl->copy_template = "<a name=\"comment\"></a>" . $tpl->copy_template;
        $rows = false;
        if ($allow_cache) $rows = xtm_cache("comm_" . $allow_cache, $this->query . " LIMIT " . $this->cstart . "," . $this->comments_per_pages);
        if ($rows) {
        ...
        if (count($rows)) foreach ($rows as $row) {
            ...
            if ($is_logged AND $edit_limit AND (($member_id['name'] == $row['name'] and $row['is_register'] and $user_group[$member_id['user_group']]['allow_delc']) or $member_id['user_group'] == '1' or $user_group[$member_id['user_group']]['del_allc'])) {
                $tpl->set('[com-del]', "<a href=\"javascript:DeleteComments('{$row['id']}', '{$xtm_login_hash}')\">");
                $tpl->set('[/com-del]', "</a>");
            ...
            if ($area == 'lastcomments') {
                ...
                if (($user_group[$member_id['user_group']]['allow_addc']) and $config['allow_comments'] == "yes") {
                    ...
                    $tpl->set('[fast]', "<a onmouseover=\"xtm_copy_quote('" . str_replace(array(" ", "&#039;"), array("&nbsp;", "&amp;#039;"), $row['name']) . "');\" href=\"#\" onclick=\"xtm_ins('" . str_replace(array(" ", "&#039;"), array("&nbsp;", "&amp;#039;"), $row['name']) . "'); return false;\">");
                    $tpl->set('[/fast]', "</a>");
            ...
            $tpl->result['comments'] = "";
            if ($area != 'ajax' AND $config['comm_msort'] == "DESC") $tpl->result['comments'] = "\n<div id=\"xtm-ajax-comments\"></div>\n";
            if ($area != 'ajax') $tpl->result['comments'] = "<form method=\"post\" action=\"\" name=\"xtmmasscomments\" id=\"xtmmasscomments\"><div id=\"xtm-comments-list\">\n" . $tpl->result['comments'];
            if ($area != 'ajax') $tpl->result['comments'] = "<a name=\"comment\"></a>" . $tpl->result['comments'];
        }
        $tpl->clear();
        if ($area != 'ajax' AND $config['comm_msort'] == "ASC") $tpl->result['comments'] .= "\n<div id=\"xtm-ajax-comments\"></div>\n";
        if ($area != 'ajax' AND $user_group[$member_id['user_group']]['del_allc'] AND !$user_group[$member_id['user_group']]['edit_limit']) $tpl->result['comments'] .= "\n<div class=\"mass_comments_action\">{$lang['mass_comments']}&nbsp;<select name=\"mass_action\"><option value=\"\">{$lang['edit_selact']}</option><option value=\"mass_combine\">{$lang['edit_selcomb']}</option><option value=\"mass_delete\">{$lang['edit_seldel']}</option></select>&nbsp;&nbsp;<input type=\"submit\" class=\"bbcodes\" value=\"{$lang['b_start']}\" /></div>\n<input type=\"hidden\" name=\"do\" value=\"comments\" /><input type=\"hidden\" name=\"xtm_allow_hash\" value=\"{$xtm_login_hash}\" /><input type=\"hidden\" name=\"area\" value=\"{$area}\" />";
        if ($area != 'ajax') $tpl->result['comments'] .= "</div></form>\n";
        if (strpos($tpl->result['content'], "<!--xtmcomments-->") !== false) {
            $tpl->result['content'] = str_replace("<!--xtmcomments-->", $tpl->result['comments'], $tpl->result['content']);
        } else {
    ...
    function build_navigation($template, $alternative_link, $link, $re_url = false)
    {
        ...
        if ($this->cstart > 1) {
            $prev = $this->cstart - 1;
            if ($config['comments_ajax'] AND $news_id) $go_page = " onclick=\"CommentsPage('{$prev}', '{$news_id}'); return false;\"";
            else $go_page = "";
        ...
        if ($this->comments_per_pages) {
            ...
            if ($enpages_count <= 10) {
                for ($j = 1; $j <= $enpages_count; $j++) {
                    if ($j != $this->cstart) {
                        if ($config['comments_ajax'] AND $news_id) $go_page = " onclick=\"CommentsPage('{$j}', '{$news_id}'); return false;\"";
                        else $go_page = "";
                        if ($config['allow_alt_url'] == "yes" AND $alternative_link) {
                ...
                if ($start >= 2) {
                    if ($config['comments_ajax'] AND $news_id) $go_page = " onclick=\"CommentsPage('1', '{$news_id}'); return false;\"";
                    else $go_page = "";
                    if ($re_url) {
                ...
                if ($this->cstart != $enpages_count) {
                    if ($config['comments_ajax'] AND $news_id) $go_page = " onclick=\"CommentsPage('{$enpages_count}', '{$news_id}'); return false;\"";
                    else $go_page = "";
                    if ($config['allow_alt_url'] == "yes" AND $alternative_link) {
        ...
        if ($this->cstart < $enpages_count) {
            ...
            if ($config['comments_ajax'] AND $news_id) $go_page = " onclick=\"CommentsPage('{$next_page}', '{$news_id}'); return false;\"";
            else $go_page = "";
            if ($config['allow_alt_url'] == "yes" AND $alternative_link) {
        ...
        $tpl->clear();
        if (strpos($tpl->result['content'], "<!--xtmnavigationcomments-->") !== false) {
            $tpl->result['content'] = str_replace("<!--xtmnavigationcomments-->", "<div class=\"xtm-comments-navigation\">" . $tpl->result['commentsnavigation'] . "</div>", $tpl->result['content']);
        } else {
            $tpl->result['content'] .= "<div class=\"xtm-comments-navigation\">" . $tpl->result['commentsnavigation'] . "</div>";
        }
    }
}
