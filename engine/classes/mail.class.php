...
class xtm_mail
{
    ...
    function xtm_mail($config, $is_html = false)
    {
    ...
    function compile_headers()
    {
        ...
        $this->mail_headers .= "X-Mailer: xtm CMS PHP" . $this->eol;
    }
    ...
    function send($to, $subject, $message)
    {
        ...
        $this->from = preg_replace("/,,/", ",", $this->from);
        if ($this->mail_method != 'smtp') $this->to = preg_replace("#\#\[\]'\"\(\):;/\$!Ј%\^&\*\{\}#", "", $this->to);
        else $this->to = '<' . preg_replace("#\#\[\]'\"\(\):;/\$!Ј%\^&\*\{\}#", "", $this->to) . '>';
