var textPadding = 16, strictDocType = true, tabView_maxNumberOfTabs = 6, xtm_tabObj = [], activeTabIndex = [], MSIE = navigator.userAgent.indexOf("MSIE") >= 0 ? true : false, navigatorVersion = navigator.appVersion.replace(/.*?MSIE (\d\.\d).*/g, "$1") / 1, tabView_countTabs = [], tabViewHeight = [], tabDivCounter = 0;
...
function initTabs(a, f, c, b, d) {
    if (!d || d == "undefined") {
        xtm_tabObj[a] = document.getElementById(a);
        b += "";
        if (b.indexOf("%") < 0) {
            b += "px"
        }
        xtm_tabObj[a].style.width = b;
        d = document.createElement("DIV");
        b = xtm_tabObj[a].getElementsByTagName("DIV")[0];
        xtm_tabObj[a].insertBefore(d, b);
        d.className = "xtm_tabPane";
        tabView_countTabs[a] = 0
    } else {
        d = xtm_tabObj[a].getElementsByTagName("DIV")[0];
        xtm_tabObj[a].getElementsByTagName("DIV");
    ...
    d = xtm_tabObj[a].getElementsByTagName("DIV");
    for (b = e = 0; b < d.length; b++) {
        if (d[b].className == "xtm_aTab") {
