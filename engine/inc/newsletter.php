...
if (isset ($_REQUEST['editor'])) $editor = $_REQUEST['editor'];
else $editor = "";
if (isset ($_REQUEST['type'])) $type = $_REQUEST['type'];
else $type = "";
if (isset ($_REQUEST['action'])) $action = $_REQUEST['action'];
else $action = "";
if (isset ($_REQUEST['a_mail'])) $a_mail = intval($_REQUEST['a_mail']);
else $a_mail = "";
if (isset ($_GET['empfanger'])) {
    ...
    if ($empfanger[0]) $empfanger = $db->safesql(implode(',', $empfanger));
    else $empfanger = "0";
} else $empfanger = "0";
if ($action == "send") {
    ...
    if ($type == "email") $type_send = $lang['bb_b_mail'];
    else $type_send = $lang['nl_pm'];
...
HTML;
    ...
    echo "if(document.addnews.message.value == '' || document.addnews.title.value == ''){ xtmalert('$lang[vote_alert]', '$lang[p_info]'); }
    else{
