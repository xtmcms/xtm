...
if (isset($_REQUEST['author'])) $author = $db->safesql(trim(htmlspecialchars($_REQUEST['author'], ENT_QUOTES, $config['charset'])));
else $author = "";
if (isset($_REQUEST['ifdelete'])) $ifdelete = $_REQUEST['ifdelete'];
else $ifdelete = "";
if (isset($_REQUEST['news_fixed'])) $news_fixed = $_REQUEST['news_fixed'];
else $news_fixed = "";
if (isset($_REQUEST['search_cat'])) $search_cat = intval($_REQUEST['search_cat']);
else $search_cat = "";
...
if ($action == "list") {
    ...
    while ($row = $db->get_array()) {
        ...
        if (xtm_strlen($row['title'], $config['charset']) > 65) $title = xtm_substr($row['title'], 0, 65, $config['charset']) . " ...";
...
function cdelete(id){
	    ...
	    xtmconfirm( '{$lang['db_confirmclear']}', '{$lang['p_confirm']}', function () {
			document.location='?mod=comments&user_hash={$xtm_login_hash}&action=dodelete&id=' + id + '';
		} );
}
...
menu[2]='<a onclick="javascript:cdelete(' + m_id + '); return(false)" href="?mod=comments&user_hash={$xtm_login_hash}&action=dodelete&id=' + m_id + '" >{$lang['comm_del']}</a>';
...
function popupedit( name ){
		...
		$("#xtmuserpopup").remove();
		$("body").append("<div id='xtmuserpopup' title='{$lang['user_edhead']}' style='display:none'></div>");
		...
		$('#xtmuserpopup').dialog({
			...
			open: function(event, ui) { 
				$("#xtmuserpopup").html("<iframe name='edituserframe' id='edituserframe' width='100%' height='400' src='{$PHP_SELF}?mod=editusers&action=edituser&user=" + name + "&rndval=" + rndval + "' frameborder='0' marginwidth='0' marginheight='0' allowtransparency='true'></iframe>");
			},
		...
		if ($(window).width() > 830 && $(window).height() > 530 ) {
			...
			$('#xtmuserpopup').dialog( "option", "position", ['0','0'] );
		}
...
HTML;
    ...
    echo "if(document.addnews.title.value == ''){ xtmalert('$lang[addnews_alert]', '$lang[p_info]'); }
    else{
    ...
    function sendNotice( id ){
		...
		b['{$lang['p_send']}'] = function() { 
			if ( $('#xtm-promt-text').val().length < 1) {
				$('#xtm-promt-text').addClass('ui-state-error');
			} else {
		...
		$('body').append(\"<div id='xtmpopup' title='{$lang['p_title']}' style='display:none'><br />{$lang['p_text']}<br /><br /><textarea name='xtm-promt-text' id='xtm-promt-text' class='ui-widget-content ui-corner-all' style='width:97%;height:100px; padding: .4em;'></textarea></div>\");
    ...
    function confirmDelete(url, id){
		...
		$(\"body\").append(\"<div id='xtmpopup' title='{$lang['p_confirm']}' style='display:none'><br /><div id='xtmpopupmessage'>{$lang['edit_cdel']}</div></div>\");
	...
	function checkxf ( )
	{
		...
		$('[uid=\"essential\"]:visible').each(function(indx) {
			...
			if($.trim($(this).find('[rel=\"essential\"]').val()).length < 1) {
				...
				xtmalert('{$lang['addnews_xf_alert']}', '{$lang['p_info']}');
		...
		if(document.addnews.title.value == ''){
			...
			xtmalert('{$lang['addnews_alert']}', '{$lang['p_info']}'); 
	...
	$(function(){
		...
		var xtm_editor = '{$config['allow_admin_wysiwyg']}';
		var launchselect = true;

		if ($.browser.opera && xtm_editor == 2 ) { launchselect = false; }
...
HTML;
    ...
    echo $output;
    if ($row['allow_comm']) $ifch = "checked";
    else $ifch = "";
    if ($row['allow_main']) $ifmain = "checked";
    else $ifmain = "";
    if ($row['approve']) $ifapp = "checked";
    else $ifapp = "";
    if ($row['fixed']) $iffix = "checked";
    else $iffix = "";
    if ($row['allow_rate']) $ifrat = "checked";
    else $ifrat = "";
    if ($row['disable_index']) $ifdis = "checked";
    else $ifdis = "";
    ...
    if ($member_id['user_group'] < 3) $disable_index = "<input type=\"checkbox\" name=\"disable_index\" value=\"1\" {$ifdis}>&nbsp;{$lang['add_disable_index']}";
    else $disable_index = "&nbsp;";
...
HTML;
    echo <<<HTML
	<div class="xtm_aTab" style="display:none;">
<table width="100%">
    <tr>
...
HTML;
    ...
    if ($row['autor'] != $member_id['name']) $notice_btn = "<input onClick=\"sendNotice('{$id}')\" type=\"button\" class=\"btn btn-inverse\" value=\"{$lang['btn_notice']}\" >&nbsp;";
    else $notice_btn = "";
    echo <<<HTML
    <tr>
        <td colspan="2"><div class="hr_line"></div></td>
...
jQuery(document).ready(function($){
	initTabs('xtm_tabView1',Array('{$lang['tabs_news']}','{$lang['tabs_vote']}','{$lang['tabs_extra']}','{$lang['tabs_perm']}'),0, '100%',0);
});
	</script>
<div style="padding-left:150px;padding-top:5px;padding-bottom:5px;">
	...
	{$notice_btn}
	<input onClick="confirmDelete('$PHP_SELF?mod=editnews&action=doeditnews&ifdelete=yes&id=$id&user_hash=$xtm_login_hash', '{$id}')" type="button" class="btn btn-danger" value="{$lang['edit_dnews']}" style="width:100px;">
...
HTML;
    ...
    if (xtm_strlen($title, $config['charset']) > 255) {
    ...
    $catalog_url = $db->safesql(xtm_substr(htmlspecialchars(strip_tags(stripslashes(trim($_POST['catalog_url']))), ENT_QUOTES, $config['charset']), 0, 3, $config['charset']));
    if ($config['create_catalog'] AND !$catalog_url) $catalog_url = $db->safesql(xtm_substr(htmlspecialchars(strip_tags(stripslashes(trim($title))), ENT_QUOTES, $config['charset']), 0, 1, $config['charset']));
