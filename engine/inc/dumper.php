...
class dumper
{
    ...
    function backup()
    {
        ...
        $this->fn_write($fp, "#xtm|{$config['version_id']}\n\n");
                ...
                while (($result = mysql_query("SELECT * FROM `{$table}` LIMIT {$from}, {$limit}")) && ($total = mysql_num_rows($result))) {
                    while ($row = mysql_fetch_row($result)) {
                        ...
                        for ($k = 0; $k < $fields; $k++) {
                            ...
                            else $row[$k] = isset($row[$k]) ? "'" . mysql_real_escape_string($row[$k]) . "'" : "NULL";
                        }
    ...
    function restore()
    {
        ...
        while (($str = $this->fn_read_str($fp)) !== false) {
            if (empty($str) || preg_match("/^(#|--)/", $str)) {
                if (!$is_xtm AND !empty($str)) {
                    $xtm_info = explode("|", $str);
                    if ($xtm_info[0] == "#xtm" AND $xtm_info[1] == $config['version_id']) $is_xtm = 1;
                    else {
            ...
            if (!$insert && preg_match("/^(INSERT INTO `?([^` ]+)`? .*?VALUES)(.*)$/i", $str, $m)) {
                if ($table != $m[2]) {
                    ...
                    $last_showed = $table;
                    $i = 0;
                    if ($is_skd) echo tpl_s(100, $t / $info[4]);
                }
                $insert = $m[1] . ' ';
                $sql .= $m[3];
                $index++;
