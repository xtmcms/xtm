...
HTML;
    ...
    while ($row = $db->get_row()) {
        $found = true;
        if ($row['date']) $date = date("d.m.Y H:i", $row['date']) . "<br /><br />";
        else $date = "";
...
$(function(){
		...
		$('.dellink1').click(function(){
		    ...
		    xtmconfirm( '{$lang['opt_complaint_5']}', '{$lang['p_confirm']}', function () {

				document.location='?mod=complaint&user_hash={$xtm_login_hash}&action=delete&id=' + id_comp + '';
...
HTML;
    ...
    while ($row = $db->get_row()) {
        ...
        $from = "<a onclick=\"javascript:popupedit('" . urlencode($row['from']) . "'); return(false)\" href=\"#\">{$row['from']}</a><br /><br /><a href=\"" . $config['http_home_url'] . "index.php?do=pm&doaction=newpm&username=" . urlencode($row['from']) . "\" target=\"_blank\">{$lang['send_pm']}</a>";
        if ($row['is_register']) $to = "<a onclick=\"javascript:popupedit('" . urlencode($row['autor']) . "'); return(false)\" href=\"#\">{$row['autor']}</a>, <a href=\"" . $config['http_home_url'] . "index.php?do=pm&doaction=newpm&username=" . urlencode($row['autor']) . "\" target=\"_blank\">{$lang['send_pm']}</a>";
        else $to = $row['autor'];
...
function DeleteComments(id) {
    ...
    xtmconfirm( '{$lang['opt_complaint_13']}?', '{$lang['p_confirm']}', function () {

		ShowLoading('');
	
		$.get("engine/ajax/deletecomments.php", { id: id, xtm_allow_hash: '{$xtm_login_hash}' }, function(r){
			...
			if (!isNaN(r)) {
				...
				xtmalert('$lang[opt_complaint_14]', '$lang[p_info]');
