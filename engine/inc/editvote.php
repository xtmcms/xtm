...
HTML;
 ...
 <!-- begin
    function confirmdelete(id){
	    xtmconfirm( '{$lang['vote_confirm']}', '{$lang['p_confirm']}', function () {
			document.location=\"?mod=editvote&action=delete&user_hash={$xtm_login_hash}&id=\"+id;
		} );
    }
    function confirmclear(id){
	    xtmconfirm( '{$lang['vote_clear']}', '{$lang['p_confirm']}', function () {
			document.location=\"?mod=editvote&action=clear&user_hash={$xtm_login_hash}&id=\"+id;
		} );
    }
 // end -->
 </script>";
    ...
    while ($row = $db->get_row()) {
        ...
        if (xtm_strlen($title, $config['charset']) > 74) {
            $title = xtm_substr($title, 0, 70, $config['charset']) . " ...";
        }
   ...
   <tr>
    ...
    <td class=\"list\" align=\"center\"><img src=\"lib/skins/images/" . $status . "\" title=\"" . $lang['led_title'] . "\" border=\"0\"></td>
    <td class=\"list\" align=\"center\">{$row['vote_num']}</td>
    <td class=\"list\" align=\"center\">{$item_category}</td>
    <td class=\"list\" align=\"center\"><a onClick=\"return dropdownmenu(this, event, MenuBuild('" . $item_id . "', '" . $led_action . "'), '170px')\" href=\"#\"><img src=\"lib/skins/images/browser_action.gif\" border=\"0\"></a></td>
     </tr>
...
menu[1]='<a onClick="document.location=\'?mod=editvote&action=' + led_action + '&user_hash={$xtm_login_hash}&id=' + m_id + '\'; return(false)" href="#">' + lang_action + '</a>';
