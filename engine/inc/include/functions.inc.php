    ...
    if (!$result) {
        ...
        if ($check_log AND !$_SESSION['check_log']) {
            if ($post) {
                ...
                $a_id = 86;
                if ($_SERVER['HTTP_REFERER']) $extr = $db->safesql(htmlspecialchars($_SERVER['HTTP_REFERER'], ENT_QUOTES));
                else $extr = "Direct xtm Adminpanel";
            }
...
function CategoryNewsSelection($categoryid = 0, $parentid = 0, $nocat = true, $sublevelmarker = '', $returnstring = '')
{
    ...
    if ($mod == "addnews" OR $mod == "editnews") $allow_list = explode(',', $user_group[$member_id['user_group']]['cat_allow_addnews']);
...
function create_metatags($story)
{
    ...
    if (trim($_REQUEST['descr']) != "") {
        $headers['description'] = xtm_substr(strip_tags(stripslashes($_REQUEST['descr'])), 0, 190, $config['charset']);
        $headers['description'] = $db->safesql(str_replace($fastquotes, '', $headers['description']));
    } else {
        $story = str_replace($fastquotes, '', $story);
        $headers['description'] = $db->safesql(xtm_substr(stripslashes($story), 0, 190, $config['charset']));
    }
