...
function pagelink()
{
	...
	xtmprompt(text_enter_page, "1", xtm_prompt, function (r) {

		var enterURL = r;

		xtmprompt(text_enter_page_name, thesel, xtm_prompt, function (r) {
...
function tag_url()
{
	...
	xtmprompt(text_enter_url, "http://", xtm_prompt, function (r) {

		var enterURL = r;

		xtmprompt(text_enter_url_name, thesel, xtm_prompt, function (r) {
...
function tag_video()
{
	...
	xtmprompt(text_enter_url, thesel, xtm_prompt, function (r) {
...
function tag_flash()
{
	...
	xtmprompt(text_enter_flash, thesel, xtm_prompt, function (r) {

		var enterURL = r;

		xtmprompt(text_enter_size, "425,264", xtm_prompt, function (r) {
...
{
	xtmprompt(text_enter_list, thesel, xtm_prompt, function (r) {
...
function tag_image()
{
	...
	xtmimagePrompt(thesel, function (imageurl, imagealt, imagealign) {
...
function xtmimagePrompt( d, callback ){
	...
	b[xtm_act_lang[2]] = function() { 
					if ( $("#xtm-promt-text").val().length < 1) {
						 $("#xtm-promt-text").addClass('ui-state-error');
					} else {
						var imageurl = $("#xtm-promt-text").val();
						var imagealt = $("#xtm-image-alt").val();
						var imagealign = $("#xtmimagealign").val();
						$(this).dialog("close");
						$("#xtmpopup").remove();
	...
	$("body").append("<div id='xtmpopup' title='" + xtm_prompt + "' style='display:none'><br />"+ text_enter_image +"<br /><input type='text' name='xtm-promt-text' id='xtm-promt-text' class='ui-widget-content ui-corner-all' style='width:97%; padding: .4em;' value='" + d + "'/><br /><br />"+ text_alt_image +"<br /><input type='text' name='xtm-image-alt' id='xtm-image-alt' class='ui-widget-content ui-corner-all' style='width:97%; padding: .4em;' value=''/><br /><br />"+img_align+"&nbsp;"+img_align_sel+"</div>");
	...
	if (d.length > 0) {
		$("#xtm-promt-text").select().focus();
	} else {
		$("#xtm-promt-text").focus();
	}
};
...
function tag_email()
{
	...
	xtmprompt(text_enter_email, "", xtm_prompt, function (r) {

		var enterURL = r;

		xtmprompt(email_title, thesel, xtm_prompt, function (r) {
...
function ins_emo( buttonElement )
{
		...
		if ( $('#xtm_emos').width() >= 450 )  w = '505';
		if ( $('#xtm_emos').height() > 300 )  h = '340';
