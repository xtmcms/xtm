...
if ($_GET['userdir']) $userdir = totranslit($_GET['userdir'], true, false) . "/";
else $userdir = "";
if ($_GET['sub_dir']) $sub_dir = totranslit($_GET['sub_dir'], true, false) . "/";
else $sub_dir = "";
...
echo "<input type=\"hidden\" name=\"action\" value=\"doimagedelete\"><input type=\"hidden\" name=\"user_hash\" value=\"$xtm_login_hash\" />
</form>
<tr height=\"1\">
...
while ($entryname = readdir($current_dir)) {
    if (is_dir(ROOT_DIR . "/uploads/$entryname") AND ($entryname != "." and $entryname != ".." and $entryname != "files")) {
        ...
        elseif ($entryname == "thumbs") $listname = $lang['images_thumb']; elseif ($entryname == "posts") $listname = $lang['images_news']; else $listname = $entryname;
