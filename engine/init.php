...
if (isset($_REQUEST['month'])) $month = @$db->safesql(strip_tags(str_replace('/', '', $_GET['month'])));
else $month = '';
if (isset($_REQUEST['day'])) $day = @$db->safesql(strip_tags(str_replace('/', '', $_GET['day'])));
else $day = '';
if (isset($_REQUEST['news_name'])) $news_name = @$db->safesql(strip_tags(str_replace('/', '', $_GET['news_name'])));
else $news_name = '';
if (isset($_REQUEST['newsid'])) $newsid = intval($_GET['newsid']);
else $newsid = 0;
...
if (isset($_REQUEST['catalog'])) {
    ...
    $catalog = $db->safesql(xtm_substr($catalog, 0, 3, $config['charset']));
} else $catalog = '';
...
if (!is_array($banned_info)) {
    ...
    while ($row = $db->get_row()) {
        if ($row['users_id']) {
            ...
            if (count(explode(".", $row['ip'])) == 4) $banned_info['ip'][$row['ip']] = array('ip' => $row['ip'], 'descr' => stripslashes($row['descr']), 'date' => $row['date']);
            elseif (strpos($row['ip'], "@") !== false) $banned_info['email'][$row['ip']] = array('email' => $row['ip'], 'descr' => stripslashes($row['descr']), 'date' => $row['date']); else $banned_info['name'][$row['ip']] = array('name' => $row['ip'], 'descr' => stripslashes($row['descr']), 'date' => $row['date']);
        }
    }
...
if ($category_skin != "") {
    ...
    if ($_REQUEST['skin_name'] != '' AND @is_dir(ROOT_DIR . '/templates/' . $_REQUEST['skin_name'])) {
        ...
        set_cookie("xtm_skin", $_REQUEST['skin_name'], 365);
    }
} elseif (isset($_COOKIE['xtm_skin'])) {
    $_COOKIE['xtm_skin'] = trim(totranslit($_COOKIE['xtm_skin'], false, false));
    if ($_COOKIE['xtm_skin'] != '' AND @is_dir(ROOT_DIR . '/templates/' . $_COOKIE['xtm_skin'])) {
        $config['skin'] = $_COOKIE['xtm_skin'];
    }
}
...
elseif (isset($do)) $do = totranslit($do); else $do = "";
if (!isset($subaction) AND isset($_REQUEST['subaction'])) $subaction = totranslit($_REQUEST['subaction']);
else $subaction = totranslit($subaction);
if (isset($_REQUEST['doaction'])) $doaction = totranslit($_REQUEST['doaction']);
else $doaction = "";
if ($do == "tags" AND !$_GET['tag']) $do = "alltags";
$xtm_module = $do;
if ($do == "" and !$subaction and $year) $xtm_module = "date";
elseif ($do == "" and $catalog) $xtm_module = "catalog"; elseif ($do == "") $xtm_module = $subaction;
if ($subaction == '' AND $newsid) $xtm_module = "showfull";
$xtm_module = $xtm_module ? $xtm_module : "main";
...
if (isset($_POST['set_new_sort']) and $config['allow_change_sort']) {
    ...
    if (in_array($_POST['xtmnewssortby'], $allowed_sort)) {
        if ($_POST['xtmdirection'] == "desc" or $_POST['xtmdirection'] == "asc") {
            $_SESSION[$find_sort] = $_POST['xtmnewssortby'];
            $_SESSION[$direction_sort] = $_POST['xtmdirection'];
            $_SESSION['xtm_no_cache'] = "1";
        }
    }
}
if ($config['allow_registration'] == "yes") {
    ...
    if ($is_logged) {
        set_cookie("xtm_newpm", $member_id['pm_unread'], 365);
        if ($member_id['pm_unread'] > intval($_COOKIE['xtm_newpm']) AND !$smartphone_detected) {
